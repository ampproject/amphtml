...
<table>
  ...
  <tr>
    <td><code>timeUpdate (low-trust)</code></td>
    <td>Fired when the playing position of a video has changed. Frequency of the event is controlled by AMP and is currently set at 1 second. This event is low-trust which means it can not trigger most actions, only low-trust actions such as <code>amp-animation</code> actions can be run.</td>
