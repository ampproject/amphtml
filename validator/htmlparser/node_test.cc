...
TEST(NodeTest, BasicStackFunctionality) {
  NodeStack stack;
  // Push three nodes.
  stack.Push(Node::make_node(NodeType::TEXT_NODE));
  stack.Push(Node::make_node(NodeType::COMMENT_NODE));
  stack.Push(Node::make_node(NodeType::ELEMENT_NODE));
  ...
  stack.Insert(1, Node::make_node(NodeType::DOCTYPE_NODE));
  stack.Insert(stack.size() - 1, Node::make_node(NodeType::DOCUMENT_NODE));
  ...
  stack.Replace(1, Node::make_node(NodeType::ERROR_NODE));
  // No change in size.
  EXPECT_EQ(5, stack.size());
...
TEST(NodeTest, NodeInnerText) {
  NodePtr doc = Parse("<html><body><div>Hello world<div>foo bar</div></div>"
