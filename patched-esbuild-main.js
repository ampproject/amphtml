...
var ensureServiceIsRunning = () => {
  ...
  child.on('close', (code, signal) => {console.log('child closed'), console.dir({code, signal});});
  child.on('disconnect', () => {console.log('child disconnected');});
  child.on('error', (err) => {console.log('child errored'); console.dir({err});});
  child.on('exit', (code, signal) => {console.log('child exited'), console.dir({code, signal});});
  child.on('message', (message, sendHandle) => {console.log('child messaged'), console.dir({message, sendHandle});});
  child.on('spawn', (code, signal) => {console.log('child spawned'), console.dir({code, signal});});
  process.on('error', (error) => {
    console.error('Process-level error', error);
  });
