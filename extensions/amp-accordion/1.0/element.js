...
export {BentoAccordion as Component} from './component';
 ...
 * @return {!JsonObject}
 */
export function elementInit(element, mutateProps, triggerEvent) {
  const getExpandStateTrigger = (section) => (expanded) => {
    toggleAttribute(section, 'expanded', expanded);
    section[SECTION_POST_RENDER]?.();
    triggerEvent(section, expanded ? 'expand' : 'collapse');
  };

  const mu = new MutationObserver(() => {
    mutateProps(getState(element, mu, getExpandStateTrigger));
  });
  mu.observe(element, {
 ...
 * @return {!JsonObject}
 */
export function getState(element, mu, getExpandStateTrigger) {
