...
Contoh di bawah ini menunjukkan penggunaan komponen web `<bento-mathml>`.
...
Contoh di bawah ini berisi `bento-mathml` dengan tiga bagian. Atribut `expanded` pada bagian ketiga memperluasnya pada pemuatan halaman.
...
Menentukan rumus (formula) yang akan dirender.
...
Contoh di bawah ini menunjukkan penggunaan `<BentoMathml>` sebagai komponen fungsional yang dapat digunakan dengan perpustakaan Preact atau React.
