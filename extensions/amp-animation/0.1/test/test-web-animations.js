...
describes.realWin('MeasureScanner', {amp: 1}, env => {
  ...
  describe.configure().skipSafari().run('MeasureScanner', () => {
    let win, doc;
    let vsync;
    let resources;
    ...
    it('should parse/validate timing duration', () => {
      ...
      expect(warnStub).to.not.be.calledWith(
          sinon.match.any, sinon.match(arg => {
    ...
    it('should parse/validate timing iterations', () => {
      ...
      expect(scanTiming({iterations: 'Infinity'}).iterations).to.equal(
          Infinity);
      expect(scanTiming({iterations: 'infinite'}).iterations).to.equal(
          Infinity);
      expect(scanTiming({iterations: 'INFINITE'}).iterations).to.equal(
          Infinity);
    ...
    it('should accept multi-animation array with ' +
        'some disabled elements', () => {
      const requests = scan([
    ...
    it('should override vars in subtargets with index', () => {
      ...
      expect(requests[1].keyframes.transform[1]).to.equal(
          'translate(32px,34px)');
    });
    ...
    describe('CSS evaluations', () => {
      ...
      it('should extract vars', () => {
        ...
        const isCssVarSupported =
            target1.style.getPropertyValue('--var0') == 'A';
