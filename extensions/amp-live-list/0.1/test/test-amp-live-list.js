...
describes.realWin(
  ...
  function(env) {
    ...
    it('has a minimum interval', () => {
      ...
      expect(liveList.getInterval()).to.equal(
    ...
    it(
      'should use actual initial items count instead of ' +
      ...
      () => {
        ...
        buildElement(elem, dftAttrs);
        liveList.buildCallback();
        expect(liveList.maxItemsPerPage_).to.not.equal(
          LiveListManager.getMinDataMaxItemsPerPage()
        );
        expect(liveList.maxItemsPerPage_).to.equal(19);
      }
    );
    ...
    describe('#update', () => {
      ...
      it('should have correct insertion by using data-sort-time', () => {
        ...
        expect(liveList.pendingItemsInsert_[0].getAttribute('id')).to.equal(
          'unique-id-num-5'
        );
        expect(liveList.pendingItemsInsert_[1].getAttribute('id')).to.equal(
          'unique-id-num-3'
        );
        expect(liveList.pendingItemsInsert_[2].getAttribute('id')).to.equal(
          'unique-id-num-4'
        );
        return liveList.updateAction_().then(() => {
          // Insertion order should be 4, 3, 5
          expect(liveList.itemsSlot_.children[0].getAttribute('id')).to.equal(
            'unique-id-num-4'
          );
          expect(liveList.itemsSlot_.children[1].getAttribute('id')).to.equal(
            'unique-id-num-3'
          );
          expect(liveList.itemsSlot_.children[2].getAttribute('id')).to.equal(
            'unique-id-num-5'
          );
    ...
    it('should have the correct "update time"', () => {
      ...
      const fromServer1 = createFromServer([{id: 'id1', sortTime: 129}]);
    ...
    it('should always use latest update to replace when in pending state', () => {
      ...
      expect(liveList.pendingItemsReplace_).to.have.length(1);
      expect(liveList.pendingItemsReplace_[0].getAttribute('id')).to.equal(
        'id1'
      );
      expect(
        liveList.pendingItemsReplace_[0].getAttribute('data-update-time')
      ).to.equal('125');
      // Should wait for user action until `updateAction_`
      expect(spy).to.have.not.been.called;

      const fromServer2 = createFromServer([{id: 'id1', updateTime: 127}]);
      liveList.update(fromServer2);
    ...
    it('should replace pagination section', () => {
      ...
      return liveList.updateAction_().then(() => {
        ...
        const fromServer2 = createFromServer([{id: 'id6'}], newPagination2);
        liveList.update(fromServer2);
    ...
    it('should choose tombstone over insert', () => {
      ...
      const fromServer1 = createFromServer([{id: 'id3', tombstone: null}]);
      expect(liveList.pendingItemsTombstone_).to.have.length(0);
      expect(liveList.pendingItemsInsert_).to.have.length(0);
    ...
    it('should delete old items to accomodate new items', () => {
      ...
      return liveList.updateAction_().then(() => {
        ...
        expect(removeChildSpy).to.have.been.called;
    ...
    it('should only delete items below the viewport', () => {
      ...
      return liveList.updateAction_().then(() => {
        ...
        expect(liveList.itemsSlot_.childElementCount).to.equal(5);
        expect(liveList.itemsSlot_.children[0].getAttribute('id')).to.equal(
          'id6'
        );
        expect(liveList.itemsSlot_.children[1].getAttribute('id')).to.equal(
        ...
        expect(liveList.itemsSlot_.children[3].getAttribute('id')).to.equal(
          'id3'
        );
        expect(liveList.itemsSlot_.children[4].getAttribute('id')).to.equal(
          'id2'
        );
    ...
    it('should keep correct count of live items', () => {
      ...
      return liveList
        ...
        .then(() => {
          ...
          expect(
            liveList.itemsSlot_.lastElementChild.getAttribute('id')
          ).to.equal('id1');
          liveList.update(fromServer);
          // Nothing removed yet
          expect(removeChildSpy).to.have.not.been.called;
        ...
        .then(() => {
          ...
          expect(
            liveList.itemsSlot_.lastElementChild.hasAttribute('data-tombstone')
          ).to.be.true;
          ...
          expect(
            liveList.itemsSlot_.lastElementChild.previousElementSibling.getAttribute(
              'id'
            )
          ).to.equal('id4');
        });
    ...
    describe('sort=ascending', () => {
      it('should have correct insertion by using data-sort-time', () => {
        ...
        const child1 = document.createElement('div');
        const child2 = document.createElement('div');
        child1.setAttribute('id', 'unique-id-num-1');
        child2.setAttribute('id', 'unique-id-num-2');
        child1.setAttribute('data-sort-time', oldDate - 2);
        ...
        const fromServer1 = createFromServer([
          {id: 'unique-id-num-4', sortTime: oldDate + 2},
          {id: 'unique-id-num-3', sortTime: oldDate + 1},
          {id: 'unique-id-num-5', sortTime: oldDate - 3},
        ]);
        ...
        return liveList.updateAction_().then(() => {
          // Insertion order should be 5, 3, 4
          expect(liveList.itemsSlot_.children[0].getAttribute('id')).to.equal(
            'unique-id-num-5'
          );
          expect(liveList.itemsSlot_.children[1].getAttribute('id')).to.equal(
          ...
          expect(liveList.itemsSlot_.children.length).to.equal(5);
        });
      });
      ...
      it('should only delete items above the viewport', () => {
        const child1 = document.createElement('div');
        const child2 = document.createElement('div');
        child1.setAttribute('id', 'id1');
        child1.textContent = 'hello world';
