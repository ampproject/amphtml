...
export class AmpLiveList extends AMP.BaseElement {
  ...
  buildCallback() {
    ...
    user().assert(Number(maxItems) > 0,
        'amp-live-list#%s must have data-max-items-per-page attribute with numeric value. Found %s', this.liveListId_,maxItems);
  ...
  validateLiveListItems_(element, opt_cacheIds) {
    ...
    user().assert(!foundInvalid,
        'All amp-live-list-items under amp-live-list#%s children must have id and data-sort-time attributes. data-sort-time must be a Number greater than 0.', this.liveListId_);
    return numItems;
  }
  ...
  getTimeAttr_(elem, attr) {
    ...
    user().assert(time > 0, '"%s" attribute must exist and value must be a number greater than 0. Found %s on %s instead.', attr,time,elem.getAttribute('id'));
    return time;
  }
