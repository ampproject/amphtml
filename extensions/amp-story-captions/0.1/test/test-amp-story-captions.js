...
describes.realWin(
  ...
  (env) => {
    ...
    it('update on video cuechange', async () => {
      await element.whenBuilt();
    ...
    it('update on audio cuechange', async () => {
      await element.whenBuilt();
      const impl = await element.getImpl();

      const audio = createElementWithAttributes(win.document, 'audio', {
        'width': '100',
        'height': '100',
        'muted': '',
        'src': BLANK_AUDIO,
      ...
      // Add captions to the audio.
      const track = audio.addTextTrack('captions', 'English', 'en');
      track.mode = 'showing';
      track.addCue(new VTTCue(0, 0.1, 'first caption'));
      track.addCue(new VTTCue(0.4, 1, 'second caption'));

      impl.setVideoElement(audio);
      expect(element.querySelector('div').textContent).to.equal(
        'first caption'
      );
      ...
      await new Promise((resolve) => {
        audio.addEventListener('timeupdate', resolve);
      });
      expect(element.querySelector('div').textContent).to.equal(
        'second caption'
      );
