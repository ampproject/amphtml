...
class AmpVideo extends AMP.BaseElement {
  ...
  buildCallback() {
    ...
    // Manage video if the sources contain bitrate or has cached sources.
    if (this.hasAnyBitrateSources_() || this.hasAnyCachedSources_()) {
  ...
  resetOnDomChange() {
    ...
    if (this.hasAnyBitrateSources_()) {
      getBitrateManager(this.win).videoChanged(this.video_, newVideo);
    }
