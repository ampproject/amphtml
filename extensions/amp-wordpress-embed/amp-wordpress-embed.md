...
[example preview="inline" playground="true" imports="amp-wordpress-embed"]

```html
<amp-wordpress-embed required-attribute>
...
The example below demonstrates `amp-wordpress-embed` component in standalone use.
...
The `amp-wordpress-embed` component API is accessible by including the following script tag in your document:

```
await customElements.whenDefined('amp-wordpress-embed-component');
const api = await WordpressEmbed.getApi();
```

The `amp-wordpress-embed` API allows you to register and respond to the following events:
...
The `amp-wordpress-embed` component has a container/non-container layout type. To ensure the component renders correctly, apply the following styles:
...
Information on how to layout and style `amp-wordpress-embed`.
...
-   `attribute-value-option-one` (default): `attribute-option-one-value` does this to `amp-wordpress-embed`.
-   `attribute-value-option-two`: `attribute-option-two-value` does this to `amp-wordpress-embed`.

### `optional-attribute-name` (optional)

Here, I write what `optional-attribute-name` will do to `amp-wordpress-embed`.

## Actions (optional)

### `action-name`

Description of action. Use cases of `action-name`. Include all the nuances, such as: `amp-wordpress-embed` needs to be identified with an `id` to work.

## Events (optional)

### `event-name`

Description of event. Use cases of event-name. Include all the nuances, such as: `amp-wordpress-embed` needs to be identified with an `id` to work.
...
<head>
  <script
    custom-element="amp-wordpress-embed"
    async
    src="https://cdn.ampproject.org/v0/amp-wordpress-embed-latest.js"
  ></script>
</head>
<body>
  <amp-wordpress-embed
...
Accessibility information related to `amp-wordpress-embed`.
