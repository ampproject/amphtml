...
function init(that) {
    const {element} = that;
    that.registerApiAction('prev', (api) => api.prev(), ActionTrust.LOW);
    that.registerApiAction('next', (api) => api.next(), ActionTrust.LOW);
    that.registerApiAction(
      'goToSlide',
      (api, invocation) => {
    ...
    return dict({
      'onSlideChange': (index) => {
        fireSlideChangeEvent(that.win, element, index, ActionTrust.HIGH);
      },
    });
  }

function isLayoutSupported(that, layout) {
  userAssert(
    isExperimentOn(that.win, 'amp-base-carousel-bento'),
    'expected amp-base-carousel-bento experiment to be enabled'
  );
  return that.super.isLayoutSupported(layout);
}
const Component = BaseCarousel;
const layoutSizeDefined = true;
const children = {
  'arrowPrev': {
    name: 'arrowPrev',
