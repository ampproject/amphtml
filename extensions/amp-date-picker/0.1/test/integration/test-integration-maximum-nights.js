...
const config = describe.configure().skipSinglePass();
...
config.run('amp-date-picker', function() {
  ...
  describes.integration('single date attribute', {
    ...
    beforeEach(() => {
      ...
      return picker.implementation_.buildCallback().then(
          () => picker.implementation_.layoutCallback());
    });
    ...
    it('should appear as blocked when a date is beyond the maximum', () => {
      ...
      const waitForStart = waitForClassName(
          startButton.parentElement, 'CalendarDay__selected_start');
      startButton.click();
...
function waitForClassName(element, className) {
  return poll(`wait for className ${className} on ${element.id}`, () => {
