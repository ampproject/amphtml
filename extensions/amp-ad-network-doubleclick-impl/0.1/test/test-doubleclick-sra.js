  ...
  allowExternalResources: true}, env => {
    ...
    describe('#constructSRABlockParameters', () => {
      it('should combine for SRA request', () => {
        ...
        sandbox.stub(impl1, 'generateAdKey_').withArgs(
            '50x320').returns('13579');
        ...
        sandbox.stub(impl2, 'generateAdKey_').withArgs(
            '250x300').returns('2468');
     ...
     */
      ...
      // Store if all elements for a given network are invalid.
        ...
        items.forEach(network => {
          ...
          expectedAttemptCollapseCalls += network.xhrFail ?
              network.instances : 0;
        });
        const grouping = {};
        ...
        return Promise.all(layoutCallbacks).then(() => expect(
            attemptCollapseSpy.callCount).to.equal(
