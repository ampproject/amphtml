...
describes.fakeWin(
  ...
  env => {
    ...
    beforeEach(() => {
      ...
      sandbox
        .stub(service.owners_, 'mutateElement')
    ...
    it('should short-circuit authorization flow when disabled', () => {
      adapterMock
        .expects('isAuthorizationEnabled')
        .withExactArgs()
        .returns(false)
        .once();
    ...
    it('should render and re-render templates when access is on', () => {
      ...
      function renderAndCheck() {
        ...
        return p.then(() => {
          ...
          expect(
            elementOn.querySelectorAll('[amp-access-template]').length
          ).to.equal(2);
  ...
  env => {
    ...
    beforeEach(() => {
      ...
      configElement.textContent = JSON.stringify({
        'authorization': 'https://acme.com/a?rid=READER_ID',
        'pingback':
          'https://acme.com/p?rid=READER_ID&type=AUTHDATA(child.type)',
        'login': 'https://acme.com/l?rid=READER_ID',
      });
      document.body.appendChild(configElement);
      document.documentElement.classList.remove('amp-access-error');
    ...
    it('should wait for last authorization completion', () => {
      ...
      return Promise.resolve()
        ...
        .then(() => {
          ...
          return Promise.all([
            service.lastAuthorizationPromises_,
        ...
        .then(() => {
          ...
          expect(
            service.analyticsEvent_.getCall(triggerStart).args[0]
    ...
    it('should NOT send analytics event if postback failed', () => {
      ...
      return service
        ...
        .then(result => {
          ...
          ).to.have.not.been.calledWith('access-pingback-sent');
          expect(service.sources_[0].analyticsEvent_).to.have.been.calledWith(
  ...
  env => {
    ...
    it('should build multiple login url', () => {
      ...
      return source.buildLoginUrls_().then(urls => {
        ...
        expect(source.loginUrlMap_['login1']).to.equal(
          'https://acme.com/l1?rid=reader1'
        );
        expect(source.loginUrlMap_['login2']).to.equal(
          'https://acme.com/l2?rid=reader1'
        );
      });
    });
    ...
    it('should open dialog in the same microtask', () => {
      ...
      expect(source.openLoginDialog_.firstCall.args[0]).to.equal(
        'https://acme.com/l?rid=R'
      );
...
describes.fakeWin(
  ...
  env => {
    ...
    beforeEach(() => {
      ...
      elementOnBeerOrDonuts.setAttribute(
        'amp-access',
