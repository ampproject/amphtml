...
export class AmpConsent extends AMP.BaseElement {
  ...
  enableInteractions_() {
    this.registerAction('accept', (invocation) => {
      this.handleClosingUiAction_(ActionType_Enum.ACCEPT, invocation);
    });

    this.registerAction('reject', (invocation) => {
      this.handleClosingUiAction_(ActionType_Enum.REJECT, invocation);
    });

    this.registerAction('dismiss', () => {
      this.handleClosingUiAction_(ActionType_Enum.DISMISS);
    });
  ...
  enableExternalInteractions_() {
    this.win.addEventListener('message', (event) => {
      ...
      if (data['info'] !== undefined) {
        ...
        if (data['action'] === ActionType_Enum.DISMISS) {
          if (data['info']) {
            this.user().error(
              TAG,
      ...
      for (let i = 0; i < iframes.length; i++) {
        if (iframes[i].contentWindow === event.source) {
          ...
          if (
            !isEnumValue(ActionType_Enum, action) ||
          ...
          if (
            ...
            action !== ActionType_Enum.DISMISS
          ) {
  ...
  handleAction_(action, consentString, opt_consentMetadata) {
    ...
    if (action == ActionType_Enum.ACCEPT) {
      //accept
    ...
    } else if (action == ActionType_Enum.REJECT) {
      // reject
      this.consentStateManager_.updateConsentInstanceState(
    ...
    } else if (action == ActionType_Enum.DISMISS) {
      this.consentStateManager_.updateConsentInstanceState(
  ...
  maybeSetConsentPurposeDefaults_(action, opt_invocation) {
    ...
    if (action === ActionType_Enum.DISMISS) {
