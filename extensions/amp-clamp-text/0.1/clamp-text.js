...
/** A marker property for when an element has been clamped. */
const CLAMPED_MARKER = '__AMP_CLAMP_CLAMPED';
...
/**
 * Clears the effects of clamping on an element, restoring content, unhiding
 * any hidden elements and hiding the overflow button.
 * @param {!Element} element
 */
function clearClamping(element) {
  // Save some time on the first call, since we do not need to clear anything
  // yet,
  if (!element[CLAMPED_MARKER]) {
    return;
  }

  element[CLAMPED_MARKER] = false;
  element.removeAttribute(CONTAINER_OVERFLOW_ATTRIBUTE);
  clearTruncation(element);
}

/**
 * Marks an element as clamped, showing the overflow button.
 * @param {!Element} element
 */
function setClamped(element) {
  element[CLAMPED_MARKER] = true;
 ...
 *    restoring on subsequent clamp calls.
...
export function clampText({
  ...
  // Set the container as clamped, so we show the overflow element and we can
  // truncate taking the size into account.
  setClamped(container);
