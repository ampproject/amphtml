 ...
 * - Delay clamping for things outside of the viewport
 * - Only clamp a few things in a single pass, and defer others
...
export class AmpClampText extends AMP.BaseElement {
  ...
  build_() {
    ...
    this.content_ = html`
      <div class="i-amphtml-clamp-content">
        <span class="i-amphtml-default-slot"></span>
        <span class="i-amphtml-clamp-expand-slot" name="expand"></span>
        <span class="i-amphtml-clamp-collapse-slot" name="collapse"></span>
      </div>
    `;

    const defaultSlot = this.content_.querySelector('.i-amphtml-default-slot');
    this.expandSlot_ = this.content_
        .querySelector('.i-amphtml-clamp-expand-slot');
    this.collapseSlot_ = this.content_
        .querySelector('.i-amphtml-clamp-collapse-slot');
  ...
  /** @override */
  layoutCallback() {
    if (isExperimentOn(this.win, 'amp-clamp-text')) {
      this.clamp_();
    }
   ...
   * we do not clamp twice as a result. TODO(sparhami) Try to find a better
   * solution for this.
   * @private
   */
  clamp_() {
    ...
    Promise.resolve()
        ...
        .then(() => {
          ...
          const overflowElement = this.useShadow_ ?
            ...
            this.element.querySelector('.i-amphtml-clamp-expand-slot');
