...
describe('adsense-a4a-config', () => {
  ...
  beforeEach(() => {
    ...
    mockWin = {
      location: parseUrlDeprecated(
        'https://nowhere.org/a/place/page.html?s=foo&q=bar'
      ),
      document: {
        querySelector: unused => {
          return null;
        },
      },
      crypto: {
  ...
  describe('#adsenseIsA4AEnabled', () => {
    ...
    it('should not enable a4a when useRemoteHtml is true', () => {
      ...
      sandbox
        .stub(urls, 'cdnProxyRegex')
        .callsFake(/^https:\/\/([a-zA-Z0-9_-]+\.)?cdn\.ampproject\.org/);
      const elem = testFixture.doc.createElement('div');
    ...
    it.skip('should not enable a4a when on a non-Google AMP cache', () => {
      ...
      sandbox
        .stub(urls, 'cdnProxyRegex')
        .callsFake(/^https:\/\/([a-zA-Z0-9_-]+\.)?amp\.cloudflare\.com/);
    ...
    Object.keys(URL_EXPERIMENT_MAPPING).forEach(expFlagValue => {
      it(`exp flag=${expFlagValue} should set eid attribute`, () => {
        ...
        if (expFlagValue == 0) {
          ...
          expect(isInExperiment(elem, URL_EXPERIMENT_MAPPING[expFlagValue])).to
            .be.true;
        }
      });
    });
