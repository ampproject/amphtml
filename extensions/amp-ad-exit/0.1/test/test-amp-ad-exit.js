...
describes.realWin('amp-ad-exit', {
  ...
  describe.configure().skipSafari().run('amp-ad-exit', () => {
    let sandbox;
    let win;
    let element;
    ...
    function addAdDiv() {
      ...
      // TODO(jonkeller): Long-term, test with amp-ad-exit enclosed inside
      // amp-ad, so we don't have to do this hack.
    ...
    beforeEach(() => {
      ...
      // TEST_3P_VENDOR must be in ANALYTICS_CONFIG *before*
      // makeElementWithConfig
    ...
    it('should ping tracking URLs with ' +
        'image requests (sendBeacon fails)', () => {
      const open = sandbox.stub(win, 'open', () => {
        return {name: 'fakeWin'};
      });
    ...
    it('border protection', () => {
      ...
      // The click is within the left border but left border protection is
      // not set.
      element.implementation_.executeAction({
        method: 'exit',
        args: {target: 'borderProtection'},
    ...
    it('should reject unrecognized 3P Analytics vendors', () => {
      ...
      unkVendor.targets.variableFrom3pAnalytics.vars._foo
          .vendorAnalyticsSource = 'nonexistent_vendor';
