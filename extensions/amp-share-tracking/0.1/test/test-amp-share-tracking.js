...
describes.fakeWin(
  ...
  env => {
    ...
    it(
      'should get incoming fragment starting with dot and ignore ' +
        'other parameters',
      () => {
        historyGetFragmentStub
          .onFirstCall()
    ...
    it(
      'should get outgoing fragment randomly if no vendor url is provided ' +
      ...
      () => {
        ...
        randomBytesStub
          .onFirstCall()
          .returns(new Uint8Array([1, 2, 3, 4, 5, 6]));
        const ampShareTracking = getAmpShareTracking();
        return Services.shareTrackingForOrNull(ampShareTracking.win).then(
          fragments => {
            expect(historyGetFragmentStub).to.be.calledOnce;
      ...
      () => {
        ...
        return Services.shareTrackingForOrNull(ampShareTracking.win).then(
          fragments => {
            ...
            // the base64url of byte array [1, 2, 3, 4, 5, 6]
            expect(fragments.outgoingFragment).to.equal('AQIDBAUG');
            expect(historyUpdateFragmentStub.withArgs('.AQIDBAUG&key=value')).to
              .be.calledOnce;
          }
        );
      }
    ...
    it(
      'should get outgoing fragment randomly if no vendor url ' +
    ...
    it(
      'should get outgoing fragment from vendor if vendor url is provided ' +
        'and the response format is correct',
      () => {
        historyGetFragmentStub.onFirstCall().returns(Promise.resolve(''));
        const mockJsonResponse = {fragment: '54321'};
        xhrStub.onFirstCall().returns(
          Promise.resolve({
            json() {
        ...
        return Services.shareTrackingForOrNull(ampShareTracking.win).then(
          fragments => {
            ...
            expect(historyUpdateFragmentStub.withArgs('.54321')).to.be
              .calledOnce;
          }
        );
      }
    ...
    it(
      'should get empty outgoing fragment if vendor url is provided ' +
        'but the response format is NOT correct',
      () => {
        expectAsyncConsoleError(
          '[amp-share-tracking] The response from ' +
    ...
    it(
      'should call fetchJson with correct request when getting outgoing ' +
        'fragment',
      () => {
        historyGetFragmentStub.onFirstCall().returns(Promise.resolve(''));
        xhrStub.onFirstCall().returns(
    ...
    it(
      ...
      () => {
        expectAsyncConsoleError(
          '[amp-share-tracking] The request to ' +
