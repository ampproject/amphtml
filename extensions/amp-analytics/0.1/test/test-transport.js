...
describe('amp-analytics.transport', () => {
  ...
  it('reuses cross-domain iframe', () => {
    ...
    sandbox.spy(transport, 'useExistingOrCreateCrossDomainIframe');
    ...
    transport.processCrossDomainIframe(window, config);
    expect(
        transport.useExistingOrCreateCrossDomainIframe.calledTwice).to.be.true;
    expect(transport.createCrossDomainIframe.calledOnce).to.be.true;
  });
  ...
  it('enqueues event messages correctly', () => {
    ...
    const queue = Transport.getFrameData(url).eventQueue;
  ...
  it('does not cause sentinel collisions', () => {
    ...
    expectAllUnique([transport.getId(), transport2.getId(), frame1.sentinel,
