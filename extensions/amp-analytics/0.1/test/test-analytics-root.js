...
describes.realWin('AmpdocAnalyticsRoot', {amp: 1}, (env) => {
  ...
  it('should add tracker, reuse and dispose', () => {
    ...
    // Reused.
    expect(
      root.getTracker(AnalyticsEventType_Enum.CUSTOM, CustomEventTracker)
    ).to.equal(tracker);
    expect(root.getTrackerOptional(AnalyticsEventType_Enum.CUSTOM)).to.equal(
      tracker
    );
    ...
    expect(root.getTrackerOptional(AnalyticsEventType_Enum.CUSTOM)).to.be.null;
  });
...
describes.realWin(
  ...
  (env) => {
    ...
    it('should add tracker, reuse and dispose', () => {
      ...
      root.dispose();
      expect(stub).to.be.calledOnce;
      expect(root.getTrackerOptional(AnalyticsEventType_Enum.CUSTOM)).to.be
        .null;
    });
    ...
    it('should create and reuse trackers, but not if not in allowlist', () => {
      ...
      expect(
        root.getTrackerForAllowlist(AnalyticsEventType_Enum.CUSTOM, allowlist)
