...
describes.realWin(
  ...
  function (env) {
    ...
    describe('Sandbox AMP Analytics Element', () => {
      ...
      it('should not add listener when eventType is not allowedlist', function () {
        expectAsyncConsoleError(clickTrackerNotSupportedError);
        // Right now we only allowedlist VISIBLE & HIDDEN
        const tracker = ins.root_.getTracker('click', ClickEventTracker);
        const addStub = env.sandbox.stub(tracker, 'add');
        const analytics = getAnalyticsTag(
          {
      ...
      it('expand vendor vars but not replace non allowedlist variables', () => {
        const analytics = getAnalyticsTag(
          {
            'requests': {
              'pageview':
      ...
      it('should not replace non allowedlist variable', () => {
        const analytics = getAnalyticsTag(
          {
            'requests': {
              'foo': 'https://example.test/cid=${clientId(analytics-abc)}',
      ...
      it('should replace allowedlist variable', () => {
        const analytics = getAnalyticsTag(
          {
      ...
      it('should replace for multi allowedlisted(or not) variables', () => {
        const analytics = getAnalyticsTag(
          {
            'requests': {
              'foo':
      ...
      it('allow a request sample through on non allowedlist url variables', () => {
        const config = {
          'requests': {
            'pageview1': '/test1=${requestCount}',
          },
