...
#### Linkers
The `linkers` configuration object is used to enable cross domain ID syncing. `amp-analytics` will use this configuration to create an unique query parameter that can be used to join sessions. This parameter will then be appended to the specified outgoing links.
...
- `paramName` - This user defined name determines the name of the created query parameter.
- `ids` - An object containing key-value pairs that will be passed along.
- `proxyOnly` - (optional) Flag indicating whether the links should only be decorated on pages served on a proxy origin. Defaults to `true`.
...
The above config would generate a parameter in this structure: `linker1=1~1clz2wh~cid~amp-SlB2gfii_11dWqtSr3zT4w~uid~1234`.
...
- The second value `1clz2wh` is a hash generated by the linker.
...
In this example configuration this parameter would be appendend to any outgoing links matching the `source` or `origin` domains. It would do this on the proxy as well as any other context.
