...
- `ids` - An object containing key-value pairs that will be passed along.
...
This linker will then use this configuration to generate a string in this structure: `<paramName>=<version>~<checkSum>~<idName1>~<idValue1>~<idName2>~<idValue2>...` The tilde `~` will be used to seperate each piece of data.
- `paramName` - Name of paramater given in configuration.
...
- `idName1~idValue1` This is where any key value pairs in the `ids` object will be included.
...
The above config would generate a parameter similar to the following: `linker1=1~1clz2wh~cid~amp-SlB2gfii_11dWqtSr3zT4w~uid~1234`.

There are several parts to this url. First you will see that the name of the param `linker1` matches the name from the configuration above. The param value contains several pieces of data seperated by the `~` delimiter.
- The first value `1` indicates the version number of the linker used.
- The second value `1clz2wh` is a checksum generated by the linker.
- After that you will see the two key value pairs given in the `ids` object. The linker has resolved any macros that may exist, and then appended them to the parameter. This results in `cid~amp-SlB2gfii_11dWqtSr3zT4w`, and `uid=1234`.

In this example configuration this parameter would be appendend to any outgoing links matching the `source` or `canonical` domains. It would do this on the proxy as well as any other context.
...
Remember that this is just the default behavior if no `destinationDomains` array is specified. You may always add addition domains to match in this array.
...
- `fingerprint` - A semi-unique value for page visitors. Generated by using `User Agent + ~ + timezone + ~ + language`. [code](https://github.com/ampproject/amphtml/blob/d8b40f62bbfe217307a21e600633343d9938aa45/extensions/amp-analytics/0.1/linker.js#L73)
- `minutesSinceEpoch` - Time since epoch. Floored to minute resolution. [code](https://github.com/ampproject/amphtml/blob/d8b40f62bbfe217307a21e600633343d9938aa45/extensions/amp-analytics/0.1/linker.js#L103)
- `key1...` - Each key value pair that will be passed along. Each key and value will be encoded. To protect against any `~` that may be within this data we `encodeURIComponent` as well as replacing the `~` with `%7E`. [code](https://github.com/ampproject/amphtml/blob/d8b40f62bbfe217307a21e600633343d9938aa45/extensions/amp-analytics/0.1/linker.js#L113)
- `CRC32` - A standard CRC32 implementation. [code](https://github.com/ampproject/amphtml/blob/master/extensions/amp-analytics/0.1/crc32.js)
- `base36` - The result of the CRC32 algorithm is base36 encoded. [code](https://github.com/ampproject/amphtml/blob/d8b40f62bbfe217307a21e600633343d9938aa45/extensions/amp-analytics/0.1/linker.js#L64)
