...
The element visibility trigger waits for element's [`ini-load`](#initial-load-trigger) signal before tracking element visibility. The signal that it waits can ben customized using the `waitFor` property in `visibilitySpec`.
...
The `visibilitySpec` is a set of conditions and properties that can be applied to `visible` or `hidden` triggers to change when they fire. If multiple properties are specified, they must all be true in order for a request to fire. Configuration properties supported in `visibilitySpec` are:
  - `waitFor` This property indicates that the visibility trigger should wait for a certain signal before track visibility. The supported values are 'none', 'ini-load' and 'render-start'. If `waitFor` is undefined, visibility trigger waits for `ini-load` if selector is specified, or wait for nothing if selector is not specified.
