...
The `visibilitySpec` is a set of conditions and properties that can be applied to `visible` or `hidden` triggers to change when they fire. If multiple properties are specified, they must all be true in order for a request to fire. Configuration properties supported in `visibilitySpec` are:
  ...
  - `repeat`: If this property is set to `true`, the trigger fires each time that the `visibilitySpec` conditions are met. In the following example, if the element is scrolled to 51% in view, then 49%, then 51% again, the trigger fires twice. However, if `repeat` was `false`, the trigger fires once. The default value of `repeat` is `false`. `reportWhen` and `repeat` may not both be used in the same visibilitySpec. Once the condition has been met, a list of visibility event [variables](https://github.com/ampproject/amphtml/blob/master/spec/amp-var-substitutions.md#visibility) will be reset. This includes `${firstSeenTime}`, `${lastSeenTime}`, `${firstVisibleTime}`, `${lastVisibleTime}`, `${maxContinuousVisibleTime}` and `${totalVisibleTime}`.
