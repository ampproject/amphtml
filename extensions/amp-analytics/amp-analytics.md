...
- `enabled` This variable can be used to instruct the analytics service to send or drop requests based on the resolved variable value. Any falsy value (`'false'`, `''`, `'0'`, `'null'`, `'NaN'` and `'undefined'`) will lead to the request being dropped.
