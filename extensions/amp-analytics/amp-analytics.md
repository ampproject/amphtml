...
The `visibilitySpec` is a set of conditions and properties that can be applied to `visible` or `hidden` triggers to change when they fire. If multiple properties are specified, they must all be true in order for a request to fire. Configuration properties supported in `visibilitySpec` are:
  ...
  - `reportWhen`: This property indicates that the visibility trigger should wait for a certain signal before sending the trigger. The only supported value is `documentExit`. `reportWhen` and `repeat` may not both be used in the same visibilitySpec. When `reportWhen` is specified, the `visibilitySpec` may include the `forceReport` property to indicate that the report should be sent at the time of the signal even if visibility requirements have not been met. In that case, any relevant variables (`totalVisibleTime`, etc.) will default to 0.
