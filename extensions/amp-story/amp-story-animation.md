...
formats:
  - stories
teaser:
  text: An animation effect definition for an AMP story.
---
...
The `<amp-story-animation>` element defines a custom animation effect in an
[`amp-story`](./amp-story.md). These effects rely on the [Web Animations API](https://www.w3.org/TR/web-animations/) to define and run animations in AMP Stories.
...
Use this attribute to chain or sequence animations. Specify the ID of the animated element that this element's animation will follow. The element must be present on the same `<amp-story-page>`. For further details, see the [Sequencing animations](./amp-story.md#sequencing-animations) section.

For example, in the following code, the `amp-story-animation` effect starts
after `object1` completes its entrance:
...
An `amp-story-animation` element defines such an animation as a JSON structure.
...
`<amp-story-animation>` is only allowed to be placed as a direct child of an `<amp-story-page>`.
...
- `--y` is not specified anywhere in the `<amp-story-animation>` and thus will be queried on the `#target1` element. It defaults to `0px` if not defined in CSS either.
...
<!--
TODO(alanorozco): This is supported on <amp-animation>, but requires additional
work and testing for <amp-story-animation>.

#### Keyframes from CSS

[Removed, see amp-animation.md for template.]
-->
...
animate quickly are allowed. This list will grow as more properties are confirmed to provide good
