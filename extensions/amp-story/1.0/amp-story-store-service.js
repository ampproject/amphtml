 ...
 * @typedef {{
 *    element: !Element,
 *    state: !EmbeddedComponentState_Enum,
 *    clientX: number,
 *    clientY: number,
 * }}
 */
...
const stateComparisonFunctions = {
  ...
  [StateProperty_Enum.SHOPPING_DATA]: (old, curr) =>
    old === null || curr === null || !deepEquals(old, curr, 2),
...
const actions = (state, action, data) => {
  switch (action) {
    ...
    case Action_Enum.ADD_SHOPPING_DATA:
      const updatedShoppingData = {
        ...state[StateProperty_Enum.SHOPPING_DATA],
        ...data,
      };
      return /** @type {!State} */ ({
        ...state,
        [StateProperty_Enum.SHOPPING_DATA]: updatedShoppingData,
      });
...
export class AmpStoryStoreService {
   ...
   * @return {!Object<StateProperty_Enum, *>} Partial state
   * @protected
   */
  getEmbedOverrides_() {
