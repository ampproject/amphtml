...
describes.realWin(
  'amp-story-page',
  {amp: {extensions: ['amp-story:1.0', 'amp-audio']}},
  (env) => {
    let win;
    let element;
    let gridLayerEl;
    ...
    it('should mark page as loaded after media is loaded', async () => {
      const waitForMediaLayoutSpy = env.sandbox.spy(
        page,
        'waitForMediaLayout_'
      );
    ...
    it('should start the animations if needed when state becomes active', async () => {
      ...
      page.buildCallback();
      await page.layoutCallback();
      const animateInStub = env.sandbox.stub(
        page.animationManager_,
        'animateIn'
      );
    ...
    it('should register amp-audio on layoutCallback', async () => {
      ...
      page.element
        .querySelector('amp-story-grid-layer')
        .appendChild(ampAudioEl);
    ...
    it('should stop the animations when state becomes not active', async () => {
      ...
      page.buildCallback();
      await page.layoutCallback();
      const cancelAllStub = env.sandbox.stub(
        page.animationManager_,
        'cancelAll'
      );
