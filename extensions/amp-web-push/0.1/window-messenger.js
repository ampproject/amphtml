...
export class WindowMessenger {
  ...
  onListenConnectionMessageReceived_(
    ...
    if (!this.isAllowedOrigin_(origin, allowedOrigins)) {
      dev().fine(TAG, 'Discarding connection message from %s because it isn\'t an allowed origin:', origin);
      return;
    }
    if (!message ||
  ...
  connect(remoteWindowContext, expectedRemoteOrigin) {
    return new Promise((resolve, reject) => {
      ...
      remoteWindowContext./*OK*/postMessage(
          ...
          [this.channel_.port2]);
      dev().fine(TAG, 'Opening channel to %s...', expectedRemoteOrigin);
    });
  }
  ...
  onChannelMessageReceived_(event) {
    ...
    if (this.messages_[message['id']] && message['isReply']) {
      ...
      if (this.debug_) {
        dev().fine(TAG, 'Received reply for topic \'%s\':', message['topic']);
      }
      promiseResolver([
        message['data'],
