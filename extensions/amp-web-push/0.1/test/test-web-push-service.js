...
describes.realWin(
  'web-push-service environment support',
  {
    amp: true,
  },
  env => {
...
describes.realWin(
  'web-push-service helper frame messaging',
  {
    amp: true,
  },
  env => {
    ...
    function setupHelperIframe() {
      ...
      return webPush.installHelperFrame(webPushConfig).then(() => {
        ...
        return webPush.frameMessenger_.connect(iframeWindow, '*');
      });
    }
...
describes.realWin(
  'web-push-service widget visibilities',
  {
    amp: true,
  },
  env => {
    ...
    it('should show blocked widget if permission query returns blocked', () => {
      ...
      return setupHelperIframe()
        ...
        .then(() => {
          expect(
            spy.withArgs(WebPushWidgetVisibilities.UNSUBSCRIBED, false)
              .calledOnce
          ).to.eq(true);
          expect(
            spy.withArgs(WebPushWidgetVisibilities.SUBSCRIBED, false).calledOnce
          ).to.eq(true);
          expect(
            spy.withArgs(WebPushWidgetVisibilities.BLOCKED, true).calledOnce
          ).to.eq(true);
        });
    });
    ...
    it('should show unsubscription widget if reachable SW returns subscribed', () => {
      ...
      return setupHelperIframe()
        ...
        .then(() => {
          ...
          expect(
            spy.withArgs(WebPushWidgetVisibilities.SUBSCRIBED, true).calledOnce
          ).to.eq(true);
          expect(
            spy.withArgs(WebPushWidgetVisibilities.BLOCKED, false).calledOnce
          ).to.eq(true);
        });
    });
    ...
    it('should show subscription widget if permission query returns default', () => {
      ...
      return setupHelperIframe()
        ...
        .then(() => {
          expect(
            spy.withArgs(WebPushWidgetVisibilities.UNSUBSCRIBED, true)
              .calledOnce
          ).to.eq(true);
          expect(
            spy.withArgs(WebPushWidgetVisibilities.SUBSCRIBED, false).calledOnce
...
describes.realWin(
  'web-push-service unsubscribing',
  {
    amp: true,
  },
  env => {
