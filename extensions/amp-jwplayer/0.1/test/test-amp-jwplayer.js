...
describes.realWin(TAG, {amp: {extensions: [TAG]}}, (env) => {
  const {buildLayoutElement, fakePostMessage} = getVideoIframeTestHelpers(
    env,
    TAG,
  ...
  beforeEach(() => {
    env.sandbox
      ...
      .returns(
        dom.createElementWithAttributes(env.win.document, 'meta', {
          property: 'og:title',
          content: 'title_tag',
        })
      );
  ...
  describe('methods', async () => {
    ...
    describe('message handling', () => {
      it('returns early if empty message', () => {
        const spy = env.sandbox.spy(impl, 'handleMessage_');
        impl.handleMessage_({});
      ...
      it("returns early if data isn't JSON or object", () => {
        const spy = env.sandbox.spy(impl, 'handleMessage_');
        impl.handleMessage_({data: 'Hello World'});
        expect(spy).returned(undefined);
      });
      ...
      it('calls onReady if valid ready message recieved', () => {
        ...
        fakePostMessage(impl.element, {event: 'ready', detail});
      ...
      it('updates duration from meta', () => {
        fakePostMessage(impl.element, {
          event: 'meta',
          detail: {metadataType: 'media', duration: 50},
        });
      ...
      it('updates mute from state', () => {
        ...
        fakePostMessage(impl.element, {event: 'mute', detail: {mute: true}});
        expect(mutedEventSpy).to.be.calledOnce;
        fakePostMessage(impl.element, {event: 'mute', detail: {mute: false}});
      ...
      it('updates current time from state', () => {
        ...
        fakePostMessage(impl.element, {event: 'time', detail: mockTime});
  ...
  it('renders with a bad playlist', async () => {
    const element = await buildLayoutElement({
