...
export function getStoryAdMetadataFromElement(adElement) {
  const ctaUrl = adElement.getAttribute(DataAttrs_Enum.CTA_URL);
  const ctaType = adElement.getAttribute(DataAttrs_Enum.CTA_TYPE);
  return {
    [A4AVarNames_Enum.CTA_TYPE]: ctaType,
    [A4AVarNames_Enum.CTA_URL]: ctaUrl,
  };
}
...
export function validateCtaMetadata(metadata, opt_inabox) {
  ...
  if (
    !metadata[A4AVarNames_Enum.CTA_TYPE] ||
    !metadata[A4AVarNames_Enum.CTA_URL]
  ) {
...
export function maybeCreateAttribution(win, metadata, container) {
  ...
  try {
    const href = metadata[A4AVarNames_Enum.ATTRIBUTION_URL];
    const src = metadata[A4AVarNames_Enum.ATTRIBUTION_ICON];
...
function createPageOutlink_(doc, uiMetadata, container) {
  ...
  const pageAnchorTag = createElementWithAttributes(
    ...
    dict({
      ...
      'target': '_top',
      'href': uiMetadata[A4AVarNames_Enum.CTA_URL],
    })
  );
  pageAnchorTag.textContent = uiMetadata[A4AVarNames_Enum.CTA_TYPE];
...
export function createCta(doc, buttonFitter, container, uiMetadata) {
  const ctaUrl = uiMetadata[A4AVarNames_Enum.CTA_URL];
  const ctaText = uiMetadata[A4AVarNames_Enum.CTA_TYPE];
