...
export function buildDom(element) {
  ...
  element.appendChild(renderContainerDom(element)); // todo
  const scrollContainer = element.querySelector('.i-amphtml-carousel-scroll');
  const prevButton = element.querySelector('.amp-carousel-button-prev');
  const nextButton = element.querySelector('.amp-carousel-button-next');
  ...
  // });
  // this.configureCarousel_(slides);
  return {scrollContainer, prevButton, nextButton};
}
 ...
 * @return {!Element}
 */
function renderContainerDom(element) {
