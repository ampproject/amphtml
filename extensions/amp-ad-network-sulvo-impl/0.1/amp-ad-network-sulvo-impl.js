...
export class AmpAdNetworkSulvoImpl extends AmpA4A {
  ...
  buildCallback() {
    ...
    const delayFetchEnabled = !!this.win.document.querySelector(
      `amp-ad[type=sulvo][${escapeCssSelectorIdent(LAZY_FETCH_ATTRIBUTE)}=true]`
    );
   ...
   * @param {!Array<!AmpAdNetworkSulvoImpl>=} instances
  ...
  getAdUrl(opt_consentTuple, opt_rtcResponsesPromise) {
    return Services.xhrFor(this.win)
      ...
      .then((jsonResp) => {
        ...
        return this._getAdUrl(opt_consentTuple, opt_rtcResponsesPromise);
      });
  }

  /**
   * Gets the Ad URL to send an XHR Request to after updating the element.
   * @param {!ConsentTupleDef=} opt_consentTuple
   * @param {Promise<!Array<rtcResponseDef>>=} opt_rtcResponsesPromise
   * @return {!Promise<string>|string}
   */
  _getAdUrl(opt_consentTuple, opt_rtcResponsesPromise) {
    if (this.useSra) {
        ...
        Object.keys(groupIdToBlocksAry).forEach((networkId) => {
          ...
          sraRequestPromises.push(
            Promise.all(blocks).then((instances) => {
              ...
              const typeInstances = /** @type {!Array<!AmpAdNetworkSulvoImpl>}*/ (instances).filter(
                (instance) => {
