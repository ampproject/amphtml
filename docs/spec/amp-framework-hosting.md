...
Important: `build-system/global-configs/custom-flavors-config.json` is not part of checked-in source and will be deleted if you run `amp clean`.
...
```sh
amp release --use_custom_configs --flavor="self-host"
```
