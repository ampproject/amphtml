...
describes.endtoend('amp-bind', {
  ...
  describe('with <amp-form>', () => {
    it('should NOT allow invalid bindings or values', async function() {
      ...
      await expect(controller.getElementAttribute(
          illegalHref, 'href')).to.be.null;
      await expect(controller.getElementAttribute(
          templatedText, 'onclick')).to.be.null;
      await expect(controller.getElementAttribute(
          templatedText, 'onmouseover')).to.be.null;
      await expect(controller.getElementAttribute(
          templatedText, 'style')).to.be.null;
      ...
      await expect(controller.getElementAttribute(
          templatedText, 'style')).to.be.null;
      await expect(controller.getElementText(
          templatedText)).to.equal('textIsLegal');
    });
  });
});
