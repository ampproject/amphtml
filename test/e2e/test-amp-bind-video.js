...
describes.endtoend('amp-bind', {
  ...
  describe('with <amp-video>', () => {
    ...
    it('should NOT change src when new value uses an ' +
        'invalid protocol', async() => {
    ...
    it('should show/hide vid controls when the control ' +
        'binding changes', async() => {
      ...
      const video = await controller.findElement('#video');
      await expect(controller.getElementAttribute(video, 'controls'))
          .to.be.null;
