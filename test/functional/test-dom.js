...
describes.sandboxed('DOM', {}, env => {
  ...
  it('templateContent on a <template> element (browser supports' +
      ' HTMLTemplateElement)', () => {
    const template = document.createElement('template');
    template.innerHTML = '<span>123</span><span>456<em>789</em></span>';
    const templateContent = dom.templateContent(template);

    const spans = templateContent.querySelectorAll('span');
    expect(spans.length).to.equal(2);
    expect(spans[0].innerHTML).to.equal('123');
    expect(spans[1].innerHTML).to.equal('456<em>789</em>');
  });

  it('templateContent on a <template> element (simulate a browser' +
