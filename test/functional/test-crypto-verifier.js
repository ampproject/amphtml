...
import {
  installCryptoVerifierService,
} from '../../src/service/crypto-verifier-impl';
import {
  decodeStringToByteArray,
  hexToByteArray,
  stringToByteArray,
} from
 '../../third_party/closure-library/test-crypto-verifier-trampolines-generated';

describe('CryptoVerifier', function() {

  const cryptoVerifier = installCryptoVerifierService(window);
  ...
  const rsaPubKey = {
    'n': new Uint8Array(decodeStringToByteArray(modulus)),
    'e': new Uint8Array(decodeStringToByteArray(pubExp))};
  const modulus1 =
  ...
  const pubExp1 = 'AQAB';
  const rsaPubKey1 = {
    'n': new Uint8Array(decodeStringToByteArray(modulus1)),
    'e': new Uint8Array(decodeStringToByteArray(pubExp1)),
  };

  const signature = new Uint8Array(hexToByteArray(
      ...
      'e50f0eef74'));
  const data = new Uint8Array(stringToByteArray('Hello'));
  const wrongData = new Uint8Array(stringToByteArray('Hello0'));


  it('should validate with the correct key and signature', () => {
    cryptoVerifier.verify(data, signature, [rsaPubKey])
  ...
  it('should not validate with the correct key but wrong data', () => {
    ...
    cryptoVerifier.verify(wrongData, signature, [rsaPubKey])
      .then(isvalid => expect(isvalid).to.be.false);
  });


  it('should not validate with the correct key but modified signature', () => {
    const modifiedSig = signature.subarray(0);
    for (let i = 0; i < signature.length ; i++) {
      modifiedSig[i] += 1;
      cryptoVerifier.verify(data, modifiedSig, [rsaPubKey, rsaPubKey])
        .then(isvalid => expect(isvalid).to.be.false);
    };
  });

  it('should not validate with wrong key', () => {
    cryptoVerifier.verify(data, signature, [rsaPubKey1])
      .then(isvalid => expect(isvalid).to.be.false);
  });

  it('should validate with 1 correct key and 1 wrong key', () => {
    cryptoVerifier.verify(data, signature, [rsaPubKey, rsaPubKey1])
      .then(isvalid => expect(isvalid).to.be.true);
  });

  it('should validate with 1 wrong key and 1 correct key', () => {
    cryptoVerifier.verify(data, signature, [rsaPubKey1, rsaPubKey1, rsaPubKey])
      .then(isvalid => expect(isvalid).to.be.true);
  });

  it('should not validate with 2 wrong keys', () => {
    cryptoVerifier.verify(data, signature, [rsaPubKey1, rsaPubKey1])
      .then(isvalid => expect(isvalid).to.be.false);
  });
});
