...
import {stringHash32, cryptoFor} from '../../src/crypto';
...
describe('XHR', function() {
  ...
  const pubKeyInfoPromise = crypto.importPublicKey(serviceName, {
    kty: 'RSA',
    'n': modulus,
    'e': pubExp,
    alg: 'RS256',
  ...
  const pubExp1 = 'AQAB';
  const pubKeyInfoPromise1 = crypto.importPublicKey(serviceName, {
    kty: 'RSA',
    'n': modulus1,
    'e': pubExp1,
    alg: 'RS256',
  ...
  describe('importPublicKey', function() {
  ...
  describes.sandboxed('verifySignature', {}, () => {
    ...
    it('should not validate with wrong key', () =>
        pubKeyInfoPromise1.then(pubKeyInfo1 =>
  ...
  describe('digest', () => {
    ...
    if (isModernChrome()) {
      it('should not call closure lib when native API is available ' +
        ...
        return new Crypto(window).sha384Base64('abc').then(hash => {
          expect(hash).to.equal(
              'ywB1P0WjXou1oD1pmsZQBycsMqsO3tFjGotgWkP_W-2AhgcroefMI1i67KE0' +
              'yCWn');
      ...
      it('should not call closure lib when native API is available ' +
        ...
        const nativeApiSpy = sandbox.spy(window.crypto.subtle, 'digest');
        const libSpy = sandbox.spy(lib, 'sha384');
        return new Crypto(window).sha384Base64(uint8Array([1,2,3])).then(
          hash => {
            expect(hash).to.equal('hiKdxtL_vqxzgHRBVKpwApHAZDUqDb3He57T8sjh2s' +
                'TcMlhn053f8dJim3o5PUf2');
            expect(nativeApiSpy).to.have.been.calledOnce;
            expect(libSpy).to.not.have.been.called;
          });
      });
