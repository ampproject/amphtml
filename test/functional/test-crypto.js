...
describes.realWin('crypto-impl', {}, env => {
  ...
  function createCrypto(win) {
    ...
    env.sandbox.stub(extensions, 'loadExtension', extensionId => {
  ...
  if (new Crypto(window).isPkcsAvailable()) {
    describe('A4A crypto', () => {
      ...
      const publicJwkPromise = keypairPromise.then(
          ({publicKey}) => subtle.exportKey('jwk', publicKey));
      const otherPublicJwkPromise = generateKeypair().then(
          ({publicKey}) => subtle.exportKey('jwk', publicKey));
      ...
      it('should create a well-formed RSA JSON Web Key (this tests the test)',
         () => {
           return publicJwkPromise.then(jwk => {
             expect(jwk['kty']).to.equal('RSA');
             expect(jwk['e']).to.equal('AQAB');
             expect(jwk['n']).to.have.lengthOf(342);
           });
         });

      describe('verifyPkcs', function() {
          ...
          return publicJwkPromise.then(jwk => crypto.importPkcsKey(jwk))
              .then(
                  key => signaturePromise.then(
                      signature => crypto.verifyPkcs(key, signature, data)))
              .then(isValid => {
        ...
        it('should not validate with the correct key but wrong data', () => {
          return publicJwkPromise.then(jwk => crypto.importPkcsKey(jwk))
              .then(
                  key => signaturePromise.then(
                      signature =>
                          crypto.verifyPkcs(key, signature, wrongData)))
              .then(isValid => {
        ...
        it('should not validate with the correct key but modified signature',
           () => {
             return publicJwkPromise.then(jwk => crypto.importPkcsKey(jwk))
                 .then(key => signaturePromise.then(signature => {
                   const arr = new Array(signature.length);
                   for (let i = 0; i < signature.length; i++) {
                     const modifiedSig = signature.slice(0);
                     modifiedSig[i] += 1;
                     arr[i] = crypto.verifyPkcs(key, modifiedSig, data)
                                  .then(isValid => {
                                    expect(isValid).to.be.false;
                                  });
                   }
        ...
        it('should not validate with wrong key', () => {
          return otherPublicJwkPromise.then(jwk => crypto.importPkcsKey(jwk))
              .then(
                  key => signaturePromise.then(
                      signature => crypto.verifyPkcs(key, signature, data)))
              .then(isValid => {
