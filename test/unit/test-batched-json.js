...
describe('batchFetchJsonFor', () => {
  ...
  beforeEach(() => {
    urlReplacements = {
      ...
      collectNotAllowedVarsSync: window.sandbox.stub(),
    };
    window.sandbox
  ...
  describe('URL replacement', () => {
    it('should not replace URL vars if opt_urlReplacement == NONE', () => {
      ...
      return batchFetchJsonFor(ampdoc, el).then(() => {
        ...
        expect(urlReplacements.collectNotAllowedVarsSync).to.not.be.called;
      });
    });
    ...
    it(
      ...
      () => {
        ...
        urlReplacements.collectNotAllowedVarsSync.withArgs(el).returns(['BAR']);
