...
describes.realWin('Resources', {amp: true}, (env) => {
  ...
  function createResource(opts) {
    const {
      ...
      isBuilt: false,
      isBuilding: false,
      state: RESOURCE_STATE_ENUM.NOT_BUILT,
      isDisplayed: true,
      isFixed: false,
      isInViewport: true,
      ...
      layoutPriority: LAYOUT_PRIORITY_ENUM.CONTENT,
      taskId: 'resource#P',
      ...opts,
    };
  ...
  it(
    ...
    () => {
      const resource = createResource({
        state: RESOURCE_STATE_ENUM.READY_FOR_LAYOUT,
      });
  ...
  it('should schedule prerenderable resource when document is in prerender', () => {
    const resource = createResource({
      ...
      layoutPriority: LAYOUT_PRIORITY_ENUM.METADATA,
    });
    resources.visible_ = false;
    env.sandbox
      .stub(resources.ampdoc, 'getVisibilityState')
  ...
  it('should not schedule prerenderable resource when document is hidden', () => {
    ...
    env.sandbox
      .stub(resources.ampdoc, 'getVisibilityState')
      .returns(VISIBILITY_STATE_ENUM.HIDDEN);
    resources.scheduleLayoutOrPreload(resource, true);
    expect(resources.queue_.getSize()).to.equal(0);
  });
  ...
  it('should update priority and schedule pass', () => {
    ...
    element.getLayoutPriority = () => LAYOUT_PRIORITY_ENUM.ADS;
    const resource = new Resource(1, element, resources);
    resources.pass_.cancel();
    expect(resource.getLayoutPriority()).to.equal(LAYOUT_PRIORITY_ENUM.ADS);

    resources.updateLayoutPriority(element, LAYOUT_PRIORITY_ENUM.METADATA);
    expect(resource.getLayoutPriority()).to.equal(
  ...
  it('should update priority and update tasks', () => {
    ...
    expect(task.priority).to.equal(LAYOUT_PRIORITY_ENUM.ADS);

    // Another element.
    const element2 = document.createElement('div');
    element2.isBuilt = () => true;
    element2.getLayoutPriority = () => LAYOUT_PRIORITY_ENUM.ADS;
    ...
    expect(task2.priority).to.equal(LAYOUT_PRIORITY_ENUM.ADS);
    ...
    expect(resources.pass_.isPending()).to.be.true;
    expect(task.priority).to.equal(LAYOUT_PRIORITY_ENUM.METADATA);
...
describes.realWin('Resources discoverWork', {amp: true}, (env) => {
  ...
  it('should measure unbuilt elements', () => {
    ...
    sandbox
      .stub(resources.ampdoc, 'getVisibilityState')
      .returns(VISIBILITY_STATE_ENUM.VISIBLE);
  ...
  it('should NOT rerender anything', () => {
    resource1.state_ = RESOURCE_STATE_ENUM.LAYOUT_COMPLETE;
    resource2.state_ = RESOURCE_STATE_ENUM.LAYOUT_COMPLETE;
    resources.visible_ = true;
    sandbox
      .stub(resources.ampdoc, 'getVisibilityState')
      .returns(VISIBILITY_STATE_ENUM.VISIBLE);
  ...
  it('should re-render from requested position', () => {
    ...
    expect(resource1.state_).to.equal(RESOURCE_STATE_ENUM.LAYOUT_COMPLETE);
    expect(resource2.state_).to.equal(RESOURCE_STATE_ENUM.LAYOUT_SCHEDULED);
  });
  ...
  it('should eject stale tasks when element unloaded', () => {
    ...
    pendingResource.state_ = RESOURCE_STATE_ENUM.NOT_BUILT;
  ...
  it('should schedule resource for execution', () => {
    ...
    expect(resource1.getState()).to.equal(RESOURCE_STATE_ENUM.LAYOUT_SCHEDULED);
  });
  ...
  it('should not schedule resource execution outside viewport', () => {
    ...
    expect(resource1.getState()).to.equal(RESOURCE_STATE_ENUM.READY_FOR_LAYOUT);
  });
  ...
  it('should build resource when not built', () => {
    ...
    resource1.state_ = RESOURCE_STATE_ENUM.NOT_BUILT;
  ...
  it('should layout resource if outside viewport but idle', () => {
    ...
    resource2.element.idleRenderOutsideViewport = () => false;
    resource1.state_ = RESOURCE_STATE_ENUM.READY_FOR_LAYOUT;
...
describes.fakeWin('Resources.add/upgrade/remove', {amp: true}, (env) => {
  ...
  function createElementWithResource(id) {
    ...
    const resource = new Resource(id, element, resources);
    resource.state_ = RESOURCE_STATE_ENUM.NOT_BUILT;
  ...
  it("remove should remove all of a resource's tasks", () => {
    const states = [
      RESOURCE_STATE_ENUM.NOT_LAID_OUT,
      RESOURCE_STATE_ENUM.READY_FOR_LAYOUT,
      RESOURCE_STATE_ENUM.LAYOUT_SCHEDULED,
    ];
