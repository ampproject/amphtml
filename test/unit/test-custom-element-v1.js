...
describes.realWin('CustomElement V1', {amp: true}, (env) => {
  ...
  describe('upgrade', () => {
    it('should not create impl immediately when attached', () => {
      ...
      expect(element.getBuildPriority()).equal(LAYOUT_PRIORITY_ENUM.CONTENT);
    ...
    it('should not upgrade immediately when attached', () => {
      ...
      expect(getImplSyncForTesting(element)).to.be.null;
      expect(element.getBuildPriority()).equal(LAYOUT_PRIORITY_ENUM.BACKGROUND);
  ...
  describe('buildInternal', () => {
    ...
    it('should build a pre-upgraded element', async () => {
      ...
      expect(element.signals().get(COMMON_SIGNALS_ENUM.BUILT)).to.be.null;
      ...
      expect(element.signals().get(COMMON_SIGNALS_ENUM.BUILT)).to.exist;
    ...
    it('should set the failing state if buildCallback fails', async () => {
      ...
      expect(element.signals().get(COMMON_SIGNALS_ENUM.BUILT)).to.exist;
      expect(
        element.signals().get(COMMON_SIGNALS_ENUM.BUILT).toString()
    ...
    it('should set the failing state if mountCallback fails', async () => {
      ...
      expect(element.readyState).to.equal('error');
      expect(element.signals().get(COMMON_SIGNALS_ENUM.MOUNTED)).to.exist;
      expect(
        element.signals().get(COMMON_SIGNALS_ENUM.MOUNTED).toString()
      ).to.match(/intentional/);
    });
  ...
  describe('setReadyStateInternal', () => {
    ...
    it('should update loading state', () => {
      ...
      expect(element.signals().get(COMMON_SIGNALS_ENUM.LOAD_START)).to.be.null;
      element.signals().signal(COMMON_SIGNALS_ENUM.UNLOAD);
      ...
      expect(element.signals().get(COMMON_SIGNALS_ENUM.LOAD_START)).to.exist;
      expect(element.signals().get(COMMON_SIGNALS_ENUM.UNLOAD)).to.be.null;
      expect(element.signals().get(COMMON_SIGNALS_ENUM.LOAD_END)).to.be.null;
    ...
    it('should update complete state', () => {
      ...
      expect(element.signals().get(COMMON_SIGNALS_ENUM.LOAD_END)).to.exist;
      expect(element.signals().get(COMMON_SIGNALS_ENUM.LOAD_START)).to.exist;
      expect(element.signals().get(COMMON_SIGNALS_ENUM.UNLOAD)).to.not.exist;
      expect(element).to.have.class('i-amphtml-layout');
    ...
    it('should update error state', () => {
      ...
      expect(element.toggleLoading).to.be.calledOnce.calledWith(false);
      expect(element.signals().get(COMMON_SIGNALS_ENUM.LOAD_END)).to.equal(
        error
      );
    ...
    it('should return back to loading after complete', () => {
      ...
      element.signals().reset(COMMON_SIGNALS_ENUM.LOAD_START);
      element.setReadyStateInternal('loading');
      expect(element.readyState).equal('loading');
      expect(element.signals().get(COMMON_SIGNALS_ENUM.LOAD_END)).to.be.null;
      expect(element.signals().get(COMMON_SIGNALS_ENUM.LOAD_START)).to.exist;
