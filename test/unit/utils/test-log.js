...
describes.sandboxed('Logging', {}, (env) => {
  const RETURNS_FINE = () => LOG_LEVEL_ENUM.FINE;
  const RETURNS_INFO = () => LOG_LEVEL_ENUM.INFO;
  const RETURNS_WARN = () => LOG_LEVEL_ENUM.WARN;
  const RETURNS_ERROR = () => LOG_LEVEL_ENUM.ERROR;
  const RETURNS_OFF = () => LOG_LEVEL_ENUM.OFF;
  ...
  describe('Level', () => {
    it('should be enabled when directly allowed', () => {
      expect(new Log(win, RETURNS_FINE).level_).to.equal(LOG_LEVEL_ENUM.FINE);
    });

    it('should be disabled when directly disallowed', () => {
      expect(new Log(win, RETURNS_OFF).level_).to.equal(LOG_LEVEL_ENUM.OFF);
    });

    it('should be disabled with no console', () => {
      win.console.log = null;
      expect(new Log(win, RETURNS_FINE).level_).to.equal(LOG_LEVEL_ENUM.OFF);
    ...
    it('should be enabled when forced for tests', () => {
      mode.test = true;
      win.ENABLE_LOG = true;
      expect(new Log(win, RETURNS_OFF).level_).to.equal(LOG_LEVEL_ENUM.FINE);
    });

    it('should be enabled as INFO when forced for localDev', () => {
      mode.localDev = true;
      expect(new Log(win, RETURNS_OFF).level_).to.equal(LOG_LEVEL_ENUM.INFO);
    });
  });
  ...
  describe('Level messages', () => {
    it('should log correctly for FINE', () => {
      ...
      expect(log.level_).to.equal(LOG_LEVEL_ENUM.FINE);
    ...
    it('should log correctly for INFO', () => {
      ...
      expect(log.level_).to.equal(LOG_LEVEL_ENUM.INFO);
    ...
    it('should log correctly for WARN', () => {
      ...
      expect(log.level_).to.equal(LOG_LEVEL_ENUM.WARN);
    ...
    it('should log correctly for ERROR', () => {
      ...
      expect(log.level_).to.equal(LOG_LEVEL_ENUM.ERROR);
    ...
    it('should report ERROR even when OFF and coallesce messages', () => {
      ...
      expect(log.level_).to.equal(LOG_LEVEL_ENUM.OFF);
      let reportedError;
  ...
  describe('UserLog', () => {
    it('should be WARN by default', () => {
      expect(user().defaultLevelWithFunc_()).to.equal(LOG_LEVEL_ENUM.WARN);
    });

    it('should be enabled in development mode', () => {
      mode.development = true;
      expect(user().defaultLevelWithFunc_()).to.equal(LOG_LEVEL_ENUM.FINE);
    });

    it('should be enabled with log=1', () => {
      win.location.hash = '#log=1';
      expect(user().defaultLevelWithFunc_(win)).to.equal(LOG_LEVEL_ENUM.FINE);
    });
  ...
  describe('DevLog', () => {
    it('should be disabled by default', () => {
      expect(dev().defaultLevelWithFunc_()).to.equal(LOG_LEVEL_ENUM.OFF);
    });
    ...
    it('should NOT be enabled with log=1', () => {
      win.location.hash = '#log=1';
      expect(dev().defaultLevelWithFunc_(win)).to.equal(LOG_LEVEL_ENUM.OFF);
    });

    it('should be enabled as INFO with log=2', () => {
      win.location.hash = '#log=2';
      expect(dev().defaultLevelWithFunc_(win)).to.equal(LOG_LEVEL_ENUM.INFO);
    });

    it('should be enabled as FINE with log=3', () => {
      win.location.hash = '#log=3';
      expect(dev().defaultLevelWithFunc_(win)).to.equal(LOG_LEVEL_ENUM.FINE);
    });
