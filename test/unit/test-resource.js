...
describes.realWin('Resource', {amp: true}, (env) => {
  ...
  beforeEach(() => {
    ...
    env.sandbox
      ...
      .callsFake(() => LAYOUT_PRIORITY_ENUM.ADS);
  ...
  it('should initialize correctly', () => {
    ...
    expect(resource.getState()).to.equal(RESOURCE_STATE_ENUM.NOT_BUILT);
  ...
  it('should build after upgraded', () => {
    ...
    return resource.build().then(() => {
      expect(resource.getState()).to.equal(RESOURCE_STATE_ENUM.NOT_LAID_OUT);
    });
  });
  ...
  it('should measure and update state', () => {
    ...
    return resource.build().then(() => {
      ...
      elementMock
        ...
        .once();
      resource.measure();
      expect(resource.getState()).to.equal(
  ...
  it('should always layout if has not been laid out before', () => {
    elementMock.expects('isUpgraded').returns(true).atLeast(1);
    resource.state_ = RESOURCE_STATE_ENUM.NOT_LAID_OUT;
    ...
    elementMock
      ...
      .once();
    resource.measure();
    expect(resource.getState()).to.equal(RESOURCE_STATE_ENUM.READY_FOR_LAYOUT);
  });

  it('should not relayout if has box has not changed', () => {
    resource.state_ = RESOURCE_STATE_ENUM.LAYOUT_COMPLETE;
    resource.layoutBox_ = {left: 11, top: 12, width: 111, height: 222};
    ...
    elementMock
      ...
      .once();
    resource.measure();
    expect(resource.getState()).to.equal(RESOURCE_STATE_ENUM.LAYOUT_COMPLETE);
  ...
  it('should not relayout if element has not completed layout', () => {
    elementMock.expects('isUpgraded').returns(true).atLeast(1);
    resource.state_ = RESOURCE_STATE_ENUM.LAYOUT_SCHEDULED;
    resource.layoutBox_ = {left: 11, top: 12, width: 111, height: 222};
    ...
    resource.measure();
    expect(resource.getState()).to.equal(RESOURCE_STATE_ENUM.LAYOUT_SCHEDULED);
    expect(resource.getLayoutBox().width).to.equal(111 + 10);
  });
  ...
  it('should ignore startLayout if already completed or failed or going', () => {
    ...
    resource.state_ = RESOURCE_STATE_ENUM.LAYOUT_FAILED;
    resource.startLayout();
  ...
  it('should fail startLayout', () => {
    ...
    return promise
      .then(
        ...
        () => {
          expect(resource.getState()).to.equal(
            RESOURCE_STATE_ENUM.LAYOUT_FAILED
          );
          expect(resource.layoutPromise_).to.equal(null);
  ...
  it('should update priority', () => {
    ...
    resource.updateLayoutPriority(LAYOUT_PRIORITY_ENUM.ADS);
    expect(resource.getLayoutPriority()).to.equal(LAYOUT_PRIORITY_ENUM.ADS);

    resource.updateLayoutPriority(LAYOUT_PRIORITY_ENUM.BACKGROUND);
    expect(resource.getLayoutPriority()).to.equal(
      LAYOUT_PRIORITY_ENUM.BACKGROUND
    );

    resource.updateLayoutPriority(LAYOUT_PRIORITY_ENUM.METADATA);
    expect(resource.getLayoutPriority()).to.equal(
      LAYOUT_PRIORITY_ENUM.METADATA
    );

    resource.updateLayoutPriority(LAYOUT_PRIORITY_ENUM.CONTENT);
    expect(resource.getLayoutPriority()).to.equal(LAYOUT_PRIORITY_ENUM.CONTENT);
  });
