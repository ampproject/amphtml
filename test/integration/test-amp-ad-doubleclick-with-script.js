...
describe('Rendering of one ad with ad script', () => {
  ...
  function replaceUrl(win) {
    ...
    const path = '/test/fixtures/doubleclick.with.script.html?google_glade=0';
  ...
  beforeEach(() => {
    ...
    return createFixtureIframe(
        'test/fixtures/doubleclick.with.script.html', 3000, win => {
          replaceUrl(win);
        }).then(f => {
          fixture = f;
        });
  ...
  it('should create an iframe loaded with ad script', function() {
    ...
    return pollForLayout(fixture.win, 1, 5500).then(() => {
      ...
      expect(pubads.get('page_url')).to.equal(
          'https://www.example.com/doubleclick.with.script.html');
