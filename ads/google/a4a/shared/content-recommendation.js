 ...
 * @enum {string}
 */
export const ExternalCorePubVars_Enum = {
...
const LAYOUT_ASPECT_RATIO_MAP = {
  [LayoutType_Enum.IMAGE_STACKED]: 1 / 1.91,
  [LayoutType_Enum.IMAGE_SIDEBYSIDE]: 1 / 3.82,
  [LayoutType_Enum.MOBILE_BANNER_IMAGE_SIDEBYSIDE]: 1 / 3.82,
  [LayoutType_Enum.PUB_CONTROL_IMAGE_STACKED]: 1 / 1.91,
  [LayoutType_Enum.PUB_CONTROL_IMAGE_SIDEBYSIDE]: 1 / 3.82,
  [LayoutType_Enum.PUB_CONTROL_IMAGE_CARD_STACKED]: 1 / 1.91,
  [LayoutType_Enum.PUB_CONTROL_IMAGE_CARD_SIDEBYSIDE]: 1 / 3.74,
  [LayoutType_Enum.PUB_CONTROL_TEXT]: 0,
  [LayoutType_Enum.PUB_CONTROL_TEXT_CARD]: 0,
};
...
const LAYOUT_TEXT_HEIGHT_MAP = {
  [LayoutType_Enum.IMAGE_STACKED]: 80,
  [LayoutType_Enum.IMAGE_SIDEBYSIDE]: 0,
  [LayoutType_Enum.MOBILE_BANNER_IMAGE_SIDEBYSIDE]: 0,
  [LayoutType_Enum.PUB_CONTROL_IMAGE_STACKED]: 80,
  [LayoutType_Enum.PUB_CONTROL_IMAGE_SIDEBYSIDE]: 0,
  [LayoutType_Enum.PUB_CONTROL_IMAGE_CARD_STACKED]: 85,
  [LayoutType_Enum.PUB_CONTROL_IMAGE_CARD_SIDEBYSIDE]: 0,
  [LayoutType_Enum.PUB_CONTROL_TEXT]: 80,
  [LayoutType_Enum.PUB_CONTROL_TEXT_CARD]: 80,
};
...
const LAYOUT_AD_WIDTH_MAP = {
  [LayoutType_Enum.PUB_CONTROL_IMAGE_STACKED]: 100,
  [LayoutType_Enum.PUB_CONTROL_IMAGE_SIDEBYSIDE]: 200,
  [LayoutType_Enum.PUB_CONTROL_IMAGE_CARD_STACKED]: 150,
  [LayoutType_Enum.PUB_CONTROL_IMAGE_CARD_SIDEBYSIDE]: 250,
  [LayoutType_Enum.PUB_CONTROL_TEXT]: 100,
  [LayoutType_Enum.PUB_CONTROL_TEXT_CARD]: 150,
};
...
export function getAutoConfig(availableWidth, isMobile) {
  if (availableWidth < MIN_PUB_CONTROL_WIDTH_OF_DESKTOP) {
    if (isMobile) {
      const layoutType = LayoutType_Enum.MOBILE_BANNER_IMAGE_SIDEBYSIDE;
      const numColumns = 1;
      const numRows = 12;
...
function validateAndParsePubControlParams(params) {
  ...
  if (numberOfPubControlParams < 3) {
    return {
      validationError: `Tags ${ExternalCorePubVars_Enum.UI_TYPE}, ${ExternalCorePubVars_Enum.COLUMNS_NUM} and ${ExternalCorePubVars_Enum.ROWS_NUM} should be set together.`,
    };
  }
  ...
  if (
    ...
    return {
      validationError: `Lengths of parameters ${ExternalCorePubVars_Enum.UI_TYPE}, ${ExternalCorePubVars_Enum.COLUMNS_NUM} and ${ExternalCorePubVars_Enum.ROWS_NUM} must match. Example: ${PUB_CONTROL_EXAMPLE}`,
    };
  }
  ...
  if (layoutTypes.length > 2) {
    return {
      validationError:
        `The parameter length of attribute ${ExternalCorePubVars_Enum.UI_TYPE}, ${ExternalCorePubVars_Enum.COLUMNS_NUM} and ${ExternalCorePubVars_Enum.ROWS_NUM} is too long. At most 2 parameters for each ` +
  ...
  for (let i = 0; i < layoutTypes.length; i++) {
    ...
    if (isNaN(row) || row === 0) {
      return {
        validationError: `Wrong value '${numberOfRows[i]}' for ${ExternalCorePubVars_Enum.ROWS_NUM}.`,
      };
    }
    ...
    if (isNaN(col) || col === 0) {
      return {
        validationError: `Wrong value '${numberOfColumns[i]}' for ${ExternalCorePubVars_Enum.COLUMNS_NUM}.`,
      };
    }
