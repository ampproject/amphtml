...
export class DomAncestorVisitor {
  ...
  constructor(win = window) {
     ...
     * @private @const {!Object<string, !Task>}
     */
    this.tasks_ = {};
   ...
   */
  addTask(taskName, callback, maxAncestorsToVisit = 100) {
    devAssert(maxAncestorsToVisit > 0,
        'maxAncestorsToVisit must be a positive value.');
    const task = new Task(callback, maxAncestorsToVisit);
    this.tasks_[taskName] = task;
    return this;
  }
   ...
   * @return {*}
   */
  getResultFor(callbackName) {
    return this.completedTasks_[callbackName].getResult();
  }
   ...
   * @return {*}
   */
  getResultForOrDefault(callbackName, defaultVal) {
    const result = this.getResultFor(callbackName);
  ...
  visitAncestorsStartingFrom(element) {
    ...
    let taskNames = Object.keys(this.tasks_);
    while (el && taskNames.length) {
      ancestorsVisited++;
      const style = computedStyle(this.win_, el);
      taskNames.forEach(taskName => {
        const task = this.tasks_[taskName];
        try {
          task.callback(el, style);
        } catch (e) {
          // TODO
        }
        if (task.isComplete() || ancestorsVisited >= task.getMaxAncestorsToVisit()) {
          this.completedTasks_[taskName] = task;
          delete this.tasks_[taskName];
        }
      });
      el = el.parentElement;
      // Some tasks might have been removed in the previous iteration, so we
      // need to reset this value.
      taskNames = Object.keys(this.tasks_);
    }
    return this;
  }
}
