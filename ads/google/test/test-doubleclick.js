...
describe('doubleclick delayed fetch white list deprecation', () => {
  ...
  it('should add experiment id', () => {
    ...
    expect(data['experimentId']).to.be.ok;
  });
  it('should append experiment id', () => {
    const data = {
      ampIsInDcdfwldExperiment: true,
      experimentId: '1000000',
      type: 'doubleclick',
    };
    doubleclick(global, data);
    expect(data['experimentId']).to.be.ok;
    expect(data['experimentId'].split(',').length).to.equal(2);
  });
  it('should not add experiment id', () => {
    ...
    doubleclick(global, data);
    expect(data['experimentId']).to.not.be.ok;
  });
});
