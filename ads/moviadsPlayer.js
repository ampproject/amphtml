...
import {camelCaseToTitleCase, getStyle, px, setStyle, setStyles} from '../src/style';
...
let gaParams = [];
let adDuration = 0;
let adDurationTotal = 0
let gaPart25 = null;
let gaPart50 = null;
let gaPart75 = null;
let imaBigPlayWidth = null;
let imaBigPlayHeight = null;
 ...
 * @param {!Object} data
 */
export function moviAdsPlayer(global, data) {
  ...
  adContainerDiv.addEventListener('click', function() {
    imaDebug('Click Ad Layer');
  });
  ...
  videoPlayer.addEventListener(
    'click',
    onScreenClick.bind(null, global)
  );
...
export function onAdProgress(global) {
  ...
  adDuration = Math.trunc(remainingMinutes * 60 + remainingSeconds);
  if (adDurationTotal === 0) {
    adDurationTotal = Math.trunc(remainingMinutes * 60 + remainingSeconds);
  }
  if (adDuration === Math.trunc(adDurationTotal * 0.75) && gaPart25 === null) {
    gaPart25 = true;
    eventGA('AD_25%', gaParams.name, gaParams.UA);
    imaDebug('AD_25%');
  }

  if (adDuration === Math.trunc(adDurationTotal * 0.5) && gaPart50 === null) {
    gaPart50 = true;
    eventGA('AD_50%', gaParams.name, gaParams.UA);
    imaDebug('AD_50%');
  }

  if (adDuration === Math.trunc(adDurationTotal * 0.25) && gaPart75 === null) {
    gaPart75 = true;
    eventGA('AD_75%', gaParams.name, gaParams.UA);
    imaDebug('AD_75%');
  }
}
...
export function playVideo() {
  controlsDiv.classList.remove('playOff');
  controlsDiv.classList.add('playOn');
...
export function pauseVideo(event = null) {
  controlsDiv.classList.remove('playOn');
  controlsDiv.classList.add('playOff');
  videoPlayer.pause();
...
export function muteVideo() {
  if (!videoPlayer.muted) {
    controlsDiv.classList.remove('muteOn');
    controlsDiv.classList.add('muteOff');
...
export function unmuteVideo() {
  if (videoPlayer.muted) {
    controlsDiv.classList.remove('muteOff');
    controlsDiv.classList.add('muteOn');
 ...
 * @visibleForTesting
 */
export function imaAdContainerStyle(event) {
  if (imaBigPlayHeight === null) {
    imaBigPlayWidth = getStyle(bigPlayDiv, 'width');
    imaBigPlayHeight = getStyle(bigPlayDiv, 'height');
  }
  if (event === 'fixed') {
    setStyles(bigPlayDiv, {
      'width': px(300),
    ...
    setStyles(bigPlayDiv, {
      'width': px(imaBigPlayWidth),
      'height': px(imaBigPlayHeight),
    });
 ...
 * @visibleForTesting
 */
export function setGaParams(params) {
  gaParams = params;
  initGA(params.UA, params.name);
}
...
/** @override */
export function initGA(ua, name) {
...
/** @override */
export function eventGA(event, name, UA) {
...
function onMessage(global, event) {
  ...
  switch (msg['func']) {
    ...
    case 'volume':
      setVolume(msg['args']);
      break;
    case 'imaAdContainerStyle':
      imaAdContainerStyle(msg['args']);
      break;
    case 'gaParams':
      setGaParams(msg['args']);
      break;
    case 'debug':
      setDebug(msg['args']);
      break;
    case 'cssPublisher':
      setCssPublisher(msg['args']);
      break;
    case 'unMute':
      unmuteVideo();
      break;
