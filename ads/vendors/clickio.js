...
import { user, dev } from '#utils/log';
...
export function clickio(global, data) {
  ...
  if (data.siteId && data.unitId) {
    ...
    if (divContainer) {
      ...
      var clickioGlobal = global[CLICKIO_GLOBAL_NAME];
      ...
      // ad code
      (global[CLICKIO_GLOBAL_COMMON_NAME]= global[CLICKIO_GLOBAL_COMMON_NAME]||{'s':{},'b':0}).cmd = global[CLICKIO_GLOBAL_COMMON_NAME].cmd || [];
        global[CLICKIO_GLOBAL_COMMON_NAME].cmd.push(function(){
        global[CLICKIO_GLOBAL_COMMON_NAME].display(div.id, "_"+data.siteId, "_"+data.unitId, data);
      });

      // load clickio script
      loadScript(global, "https://s.clickiocdn.com/t/"+data.siteId+"/360_amp.js");

      // load consent module for master
      if (context.isMaster) {
        // consent module
        if (context.initialConsentMetadata !== null
          ...
          // load consent script
          loadScript(global, "https://s.clickiocdn.com/t/static/amp/consent_amp.js");
        }
      }

      dev().info(CLICKIO_LOG_TAG, 'clickio: running ('+(context.isMaster ? 'master':'slave')+')');
    }
  } else {
