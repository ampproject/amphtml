...
export function jioads(global, data) {
  ...
  writeScript(
    global,
    'https://mercury.akamaized.net/jioads/websdk/amp/jioAds.js?rnd='+Math.random()
  );
  loadScript(
    global,
    'https://mercury.akamaized.net/jioads/websdk/amp/ampwrapper.js?rnd='+Math.random(),
    () => {
      let refresh = '';
      let adMetaData = '';
      let video = '';
      ...
      if (data.hasOwnProperty('videoAd') && data['videoAd'] == '1') {
      ...
      container.style.width = `${global.context.initialIntersection.boundingClientRect.width}px`;
      container.style.height = `${global.context.initialIntersection.boundingClientRect.height}px`; 
      html = `${video} <ins id="${global.context.container}" data-adspot-key="${data['adspot']}" data-source="${data['pkgName']}" ${refresh} ${adMetaData} ${videocontainer}></ins>`;
      container.innerHTML = html;
      global.document.getElementById('c').appendChild(container);
    },
    () => {
      console.log('fall back from wrapper');
      global.context.noContentAvailable();
    }
  );
