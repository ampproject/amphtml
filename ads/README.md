...
See also our [3rd party ads integration guidelines](/3p.md)
...
If the publisher would like to give custom javascript in the remote.html, the ability to read or write to  the publisher owned cookies, the publisher needs to ensure that the remote.html file is hosted on a sub-domain of the publisher URL. e.g. if the publisher hosts a webpage on https://nytimes.com, then the remote file should be hosted on something similar to https://sub-domain.nytimes.com for the custom JS to have the abiity to read or write to nyt’s cookies.
