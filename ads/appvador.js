...
export function appvador(global, data) {
  ...
  const scriptUrl = data.customScript ? data.customScript :
      'https://cdn.apvdr.com/js/' +
      (data.jsType ? data.jsType : 'VastAdUnit') + '.min.js';
  const apvScript = 'new APV.' + (data.jsType ? data.jsType : 'VASTAdUnit') +
      '({s:"' + data.id + '",isAmpAd:true' +
      (data.options ? (',' + data.options) : '') + '}).load();';

  const cb = function() {
    const apvLoadScript = global.document.createElement('script');
    apvLoadScript.innerHTML = apvScript;
