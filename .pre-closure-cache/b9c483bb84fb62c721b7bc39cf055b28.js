...
updateLengthStyle,
updateScrollPosition } from "./dimensions";
...
export class Carousel {
   ...
   * }=} options
   */
  advance(delta, options = {}) {let _options = options;
    const { currentIndex_, requestedIndex_, slides_ } = this;
    const { actionSource, allowWrap = false } = _options;
   ...
   * }=} options
   */
  goToSlide(index, options = {}) {let _options2 = options;
    const { actionSource, smoothScroll = true } = _options2;
   ...
   * @private
   */
  resetScrollReferencePoint_(force = false) {let _force = force;
