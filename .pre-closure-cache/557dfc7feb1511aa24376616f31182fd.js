...
export function groupAmpAdsByType(ampdoc, type, groupFn) {
  ...
  resources.map((resource) => {
    ...
    // be upgraded.
    return whenUpgradedToCustomElement( /** @type {!Element} */(
    ampAdSelector(resource)));
...
domain = undefined,
startTime = Date.now())
{let _redirectsRemaining = redirectsRemaining,_domain = domain,_startTime = startTime;
  const url = getIdentityTokenRequestUrl(win, ampDoc, _domain);
  ...
  then((obj) => {
    ...
    const fetchTimeMs = Date.now() - _startTime;
    if (IDENTITY_DOMAIN_REGEXP_.test(altDomain)) {
      if (!_redirectsRemaining--) {
        // Max redirects, log?
        return { fetchTimeMs };
      }
 ...
 * @visibleForTesting
 */
export function getIdentityTokenRequestUrl(win, ampDoc, domain = undefined) {let _domain2 = domain;
  if (!_domain2 && win != win.top && win.location.ancestorOrigins) {
    const matches = IDENTITY_DOMAIN_REGEXP_.exec(
    win.location.ancestorOrigins[win.location.ancestorOrigins.length - 1]);

    _domain2 = (matches && matches[0]) || undefined;
  }
  _domain2 = _domain2 || '.google.com';
  const canonical = extractHost(
  Services.documentInfoForDoc(ampDoc).canonicalUrl);

  return `https://adservice${_domain2}/adsid/integrator.json?domain=${canonical}`;
}
