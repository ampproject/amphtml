...
export class Xhr {
   ...
   * @private
   */
  fetchAmpCors_(input, init = {}) {let _init = init;
    input = setupInput(this.win, input, _init);
    _init = setupAMPCors(this.win, input, _init);
    return this.fetch_(input, _init).then(
    (response) => response,
    (reason) => {
