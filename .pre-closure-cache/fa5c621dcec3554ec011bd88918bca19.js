...
import { pushIfNotExist, removeItem } from "../types/array";
...
export class ContextNode {
   ...
   * @return {?ContextNode}
   */
  static closest(node, includeSelf = true) {let _includeSelf = includeSelf;
    let n = node;
    while (n) {
      // Check if a node is a candidate to be returned.
      if (n != node || _includeSelf) {
        if (n[NODE_PROP]) {
 ...
 */
function forEachContained(node, callback, includeSelf = true) {let _includeSelf2 = includeSelf;
  const closest = ContextNode.closest(node, _includeSelf2);
  if (!closest) {
    return;
  }
