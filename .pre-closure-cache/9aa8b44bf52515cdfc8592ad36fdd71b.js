...
import { deepMerge, dict, getValueForExpr, map } from "../../../src/core/types/object";
import { dev, devAssert, user } from "../../../src/log";
import { escapeCssSelectorIdent } from "../../../src/core/dom/css-selectors";
import { findIndex, isArray, remove, toArray } from "../../../src/core/types/array";
...
export class Bind {
   ...
   * @return {!Promise}
   */
  setState(state, opts = {}) {let _opts = opts;
    dev().info(TAG, 'setState (init=%s):', _opts.skipEval, state);
    ...
    then((results) =>
    this.apply_(results, {
      skipAmpState: _opts.skipAmpState,
      constrain: _opts.constrain }));
   ...
   */
  rescan(addedElements, removedElements, options = {}) {let _options = options;
    ...
    this.rescan_(addedElements, removedElements, _options),
   ...
   * @private
   */
  verify_(results, elements = null, warn = true) {let _elements = elements,_warn = warn;
    ...
    this.boundElements_.forEach((boundElement) => {
      ...
      if (_elements && !this.elementsContains_(_elements, element)) {
        return;
      }
   ...
   * @private
   */
  slowScan_(addedNodes, removedNodes, label = 'rescan.slow') {let _label = label;
    let removed = 0;
