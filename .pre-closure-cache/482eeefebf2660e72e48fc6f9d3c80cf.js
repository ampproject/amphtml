...
export class AbstractCustomValidator extends FormValidator {
  ...
  hideAllValidations() {
    const inputs = this.inputs();
    for (let i = 0; i < inputs.length; i++) {
      this.hideValidationFor( /** @type {!Element} */(inputs[i]));
    }
  }
   ...
   * @return {?Element}
   */
  getValidationFor(input, inputInvalidType = undefined) {let _inputInvalidType = inputInvalidType;
    if (!input.id) {
      return null;
    }
    const invalidType = this.getInvalidType_(input, _inputInvalidType);
   ...
   */
  getInvalidType_(input, inputInvalidType = undefined) {let _inputInvalidType2 = inputInvalidType;
    ...
    if (
    tagName === 'TEXTAREA' &&
    _inputInvalidType2 === 'customError' &&
    validationMessage === CUSTOM_PATTERN_ERROR)
    {
      return 'patternMismatch';
    }

    return _inputInvalidType2;
  }
