...
export class AmpStoryViewerMessagingHandler {
   ...
   */
  send(eventType, data, cancelUnsent = false) {let _cancelUnsent = cancelUnsent;
    this.viewer_.sendMessage(eventType, data, _cancelUnsent);
  }
   ...
   * @private
   */
  onGetDocumentState_(data = {}) {let _data = data;
    const { state } = _data;
   ...
   * @private
   */
  onOnDocumentState_(data = {}) {let _data2 = data;
    const { state } = _data2;
    const config = GET_STATE_CONFIGURATIONS[state];
   ...
   * @private
   */
  onSetDocumentState_(data = {}) {let _data3 = data;
    const { state, value } = _data3;
