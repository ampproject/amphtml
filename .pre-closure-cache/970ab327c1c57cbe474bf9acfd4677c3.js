...
export class AmpAdNetworkBase extends AMP.BaseElement {
   ...
   * @final
   */
  registerValidator(validator, type = 'default') {let _type = type;
    if (this.validators_[_type]) {
      dev().warn(TAG, `${_type} validator already registered.`);
    }
    this.validators_[_type] = validator;
  }
