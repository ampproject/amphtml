...
export class AmpAnalytics extends AMP.BaseElement {
  ...
  constructor(element) {
    ...
    /** @private {!Promise} */
    this.consentPromise_ = _resolvedPromise();
  ...
  buildCallback() {
    ...
    if (this.consentNotificationId_ != null) {
      ...
      this.element).
      then((service) =>
      service.get( /** @type {string} */(this.consentNotificationId_)));
    ...
    for (const k in this.config_['triggers']) {
      if (hasOwn(this.config_['triggers'], k)) {
        ...
        if (this.isSandbox_) {
          ...
          if (
          (eventType === "click" || eventType === "browser-event" || eventType === "custom" || eventType === "hidden" || eventType === "ini-load" || eventType === "render-start" || eventType === "scroll" || eventType === "story" || eventType === "timer" || eventType === "video" || eventType === "visible") &&
