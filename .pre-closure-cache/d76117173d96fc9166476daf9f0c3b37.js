...
hasStoredValue } from "./consent-info";

import { CSS } from "../../../build/amp-consent-0.1.css";
import {
ConsentConfig,
expandConsentEndpointUrl,
expandPolicyConfig } from "./consent-config";
...
NotificationUiManager } from "../../../src/service/notification-ui-manager";
...
import { realChildElements } from "../../../src/core/dom/query";
...
export class AmpConsent extends AMP.BaseElement {
    ...
    this.win.addEventListener('message', (event) => {
      ...
      for (let i = 0; i < iframes.length; i++) {
        if (iframes[i].contentWindow === event.source) {
          ...
          if (
          !(action === "accept" || action === "reject" || action === "dismiss") ||
  ...
  maybeSetUpTcfPostMessageProxy_() {
    ...
    if (!this.win.frames[TCF_API_LOCATOR]) {
      ...
      this.element.appendChild( /** @type {!Element} */(iframe));
    }
  }
