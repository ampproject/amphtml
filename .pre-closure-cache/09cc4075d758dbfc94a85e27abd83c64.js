...
AsyncInputClasses } from "../../../src/core/constants/async-input";
...
FORM_VERIFY_OPTOUT,
FORM_VERIFY_PARAM,
getFormVerifier } from "./form-verifiers";
...
import { ancestorElementsByTag, childElementByAttr } from "../../../src/core/dom/query";
import { createCustomEvent } from "../../../src/event-helper";
import {
createElementWithAttributes,
iterateCursor,
removeElement,
tryFocus } from "../../../src/core/dom";
...
getFormAsObject,
setFormForElement } from "../../../src/form";
...
setupAMPCors,
setupInit,
setupInput } from "../../../src/utils/xhr-utils";
...
export class AmpForm {
  ...
  installEventHandlers_() {
    ...
    'blur',
    (e) => {
      checkUserValidityAfterInteraction_( /** @type {!Element} */(e.target));
  ...
  handleSsrTemplate_(trust) {
    ...
    then(() => {
      const request = this.requestForFormFetch( /** @type {string} */(
      this.xhrAction_),
      this.method_);
   ...
   */
  doActionXhr_() {
    return this.doXhr_( /** @type {string} */(this.xhrAction_), this.method_);
  }
  ...
  doVerifyXhr_() {
    ...
    return this.doXhr_( /** @type {string} */(
    this.xhrVerify_),
    this.method_,
    ...
    if (container) {
      ...
      if (this.templates_.hasTemplate(container)) {
        ...
        then((rendered) => {
          ...
          return this.mutator_.mutateElement( /** @type {!Element} */(
          container),
          () => {
            container.appendChild( /** @type {!Element} */(renderContainer));
  ...
  maybeInitializeFromUrl_() {
    ...
    Object.keys(queryParams).forEach((key) => {
      ...
      if (formControls.nodeType === Node.ELEMENT_NODE) {
        const field = /** @type {!Element} */(formControls);
...
function removeValidityStateClasses(form) {
  ...
  for (const validityState in dummyInput.validity) {
    ...
    iterateCursor(elements, (element) => {
      /** @type {!Element} */(element).classList.remove(validityState);
    });
  }
}
 ...
 */
function checkUserValidity(element, propagate = false) {let _propagate = propagate;
