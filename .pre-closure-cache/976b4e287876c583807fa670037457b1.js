 ...
 * @return {Promise}
 */
export function addStoryPages(doc, storyImpl, numPages = 3) {let _numPages = numPages;
  const { element } = storyImpl;
  const promises = [];
  for (let i = 0; i < _numPages; i++) {
 ...
 */
export function fireBuildSignals(doc, additonalSelectors = []) {let _additonalSelectors = additonalSelectors;
  const defaultSelectors = ['amp-ad', 'amp-story', 'amp-story-page'];
  const selectors = defaultSelectors.concat(_additonalSelectors).join(',');
