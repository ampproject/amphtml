...
ref)
{let _Comp = Comp,_ContentComp = ContentComp,_layout = layout,_paint = paint,_size = size;
  // The formula: `size << 2 | layout << 1 | paint`.
  const containIndex = (_size ? 4 : 0) + (_layout ? 2 : 0) + (_paint ? 1 : 0);
  return (
    ...
    Preact.createElement(_ContentComp, { ...
      contentProps,
      ref: contentRef,
      className: contentClassName,
      style: {
        ...(((_size && SIZE_CONTENT_STYLE))),
        'overflow': _paint ? 'hidden' : 'visible',
        ...contentStyle } },
