...
export class AmpDateDisplay extends AMP.BaseElement {
   ...
   * @private
   */
  getVariablesInLocal_(date, locale, localeOptions = DEFAULT_DATETIME_OPTIONS) {let _localeOptions = localeOptions;
    return {
      ...
      localeString: this.getLocaleString_(date, locale, _localeOptions) };
   ...
   * @private
   */
  getVariablesInUTC_(date, locale, localeOptions = DEFAULT_DATETIME_OPTIONS) {let _localeOptions2 = localeOptions;
  ...
  rendered_(element) {
    this.mutateElement(() => {
      removeChildren( /** @type {!Element} */(this.container_));
