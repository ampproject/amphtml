...
ref)
{let _unloadOnPause = unloadOnPause,_sandbox = sandbox,_muted = muted,_controls = controls;
  ...
  [playerStateRef, postMethodMessage, _unloadOnPause]);
  ...
  useLayoutEffect(() => {
    postMethodMessage(_muted ? 'mute' : 'unmute');
  }, [_muted, postMethodMessage]);

  useLayoutEffect(() => {
    postMethodMessage(_controls ? 'showControls' : 'hideControls');
  }, [_controls, postMethodMessage]);
...
function VideoIframeWithRef(props, ref) {
  return Preact.createElement(VideoWrapper, { ref: ref, ...props, component: VideoIframeInternal });
}
