...
function createBaseCustomElementClass(win, elementConnectedCallback) {
  ...
  class BaseCustomElement extends htmlElement {
     ...
     */
    updateLayoutBox(layoutBox, sizeChanged = false) {let _sizeChanged = sizeChanged;
      if (this.isBuilt()) {
        this.onMeasure(_sizeChanged);
      }
    }
     ...
     */
    getImpl(waitForBuild = true) {let _waitForBuild = waitForBuild;
      const waitFor = _waitForBuild ? this.build() : this.createImpl_();
     ...
     * @public @final
     */
    toggleLoading(state, force = false) {let _force = force;
      ...
      if (loadingIndicator) {
        state = state && this.isLoadingEnabled_(_force);
        if (state) {
     ...
     */
    mutateOrInvoke_(mutator, opt_element, opt_skipRemeasure = false) {let _opt_skipRemeasure = opt_skipRemeasure;
      if (this.ampdoc_) {
