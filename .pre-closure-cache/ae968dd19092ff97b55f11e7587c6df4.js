...
export class AnalyticsRoot {
   ...
   */
  getElement(context, selector, selectionMethod = null) {let _selectionMethod = selectionMethod;
    ...
    return this.ampdoc.whenReady().then(() => {
      ...
      try {
        if (_selectionMethod == 'scope') {
          found = scopedQuerySelector(context, selector);
        } else if (_selectionMethod == 'closest') {
   ...
   */
  getElementsByQuerySelectorAll_(selectors, useDataVars = true) {let _useDataVars = useDataVars;
    // Wait for document-ready to avoid false missed searches
    return this.ampdoc.whenReady().then(() => {
      let elements = [];
   ...
   */
  getElements(context, selectors, selectionMethod, useDataVars = true) {let _useDataVars2 = useDataVars;
   ...
   */
  createSelectiveListener(listener, context, selector, selectionMethod = null) {let _selectionMethod2 = selectionMethod;
    return (event) => {
      if (selector == ':host') {
      ...
      while (target) {
        ...
        if (_selectionMethod2 == 'closest' && !target.contains(context)) {
