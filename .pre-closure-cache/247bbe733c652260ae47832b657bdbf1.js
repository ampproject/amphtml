...
export class AmpStoryPage extends AMP.BaseElement {
   ...
   * @private
   */
  findAndPrepareEmbeddedComponents_(forceResize = false) {let _forceResize = forceResize;
    this.addClickShieldToEmbeddedComponents_();
    this.resizeInteractiveEmbeddedComponents_(_forceResize);
   ...
   * @private
   */
  pauseAllMedia_(rewindToBeginning = false) {let _rewindToBeginning = rewindToBeginning;
    return this.whenAllMediaElements_((mediaPool, mediaEl) => {
      return this.pauseMedia_(
      mediaPool,
      mediaEl,
      /** @type {boolean} */(_rewindToBeginning));
   ...
   *     there isn't one.
   */
  getNextPageId(isAutomaticAdvance = false) {let _isAutomaticAdvance = isAutomaticAdvance;
    if (_isAutomaticAdvance && this.element.hasAttribute('auto-advance-to')) {
   ...
   *     by an automatic advancement after a timeout.
   */
  next(isAutomaticAdvance = false) {let _isAutomaticAdvance2 = isAutomaticAdvance;
    const pageId = this.getNextPageId(_isAutomaticAdvance2);
   ...
   * @private
   */
  stopMeasuringAllVideoPerformance_(sendMetrics = true) {let _sendMetrics = sendMetrics;
    if (!this.mediaPerformanceMetricsService_.isPerformanceTrackingOn()) {
      return;
    }
  ...
  togglePlayMessage_(isActive) {
    if (!isActive) {
      this.playMessageEl_ &&
      this.mutateElement(() =>
      toggle( /** @type {!Element} */(this.playMessageEl_), false));
    ...
    this.mutateElement(() =>
    toggle( /** @type {!Element} */(this.playMessageEl_), true));
  ...
  toggleErrorMessage_(isActive) {
    if (!isActive) {
      ...
      this.mutateElement(() =>
      toggle( /** @type {!Element} */(this.errorMessageEl_), false));
    ...
    this.mutateElement(() =>
    toggle( /** @type {!Element} */(this.errorMessageEl_), true));
   ...
   */
  openAttachment(shouldAnimate = true) {let _shouldAnimate = shouldAnimate;
    const attachmentEl = this.element.querySelector(
    'amp-story-page-attachment, amp-story-page-outlink');
    ...
    attachmentEl.getImpl().then((attachment) => attachment.open(_shouldAnimate));
  }
