...
export class OriginExperiments {
   ...
   */
  getExperiments(opt_rescan = false, publicJwk = PUBLIC_JWK) {let _opt_rescan = opt_rescan,_publicJwk = publicJwk;
    if (!this.crypto_.isPkcsAvailable()) {
      user().error(TAG, 'Crypto is unavailable.');
      return Promise.resolve([]);
    }
    if (!this.scanPromise_ || _opt_rescan) {
      this.scanPromise_ = this.scanForTokens_(_publicJwk);
    }
