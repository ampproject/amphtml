...
export class AmpStoryPlayer {
  ...
  setUpMessagingForStory_(story) {
    ...
    story.messagingPromise = new Promise((resolve) => {
      ...
      (messaging) => {
        messaging.setDefaultHandler(() => _resolvedPromise());
   ...
   * @return {!Promise}
   */
  show(storyUrl, pageId = null, options = {}) {let _pageId = pageId,_options = options;
    const story = this.getStoryFromUrl_(storyUrl);

    let renderPromise = _resolvedPromise3();
    if (story.idx !== this.currentIdx_) {
      this.currentIdx_ = story.idx;

      renderPromise = this.render_();

      if (_options.animate === false) {
    ...
    if (_pageId != null) {
      return renderPromise.then(() => this.goToPageId_(_pageId));
    }
   ...
   * @param {{animate: boolean?}} options
   */
  go(storyDelta, pageDelta = 0, options = {}) {let _pageDelta = pageDelta,_options2 = options;
    if (storyDelta === 0 && _pageDelta === 0) {
      return;
    }
    ...
    let showPromise = _resolvedPromise4();
    if (this.currentIdx_ !== newStory.idx) {
      showPromise = this.show(newStory.href, /* pageId */null, _options2);
    }

    showPromise.then(() => {
      this.selectPage_(_pageDelta);
    });
  }
   ...
   * @private
   */
  render_(startingIdx = this.currentIdx_) {let _startingIdx = startingIdx;
    const renderPromises = [];

    for (let i = 0; i < this.stories_.length; i++) {
      const story = this.stories_[(i + _startingIdx) % this.stories_.length];
   ...
   * @private
   */
  getEncodedLocation_(href, visibilityState = VisibilityState.INACTIVE) {let _visibilityState = visibilityState;
