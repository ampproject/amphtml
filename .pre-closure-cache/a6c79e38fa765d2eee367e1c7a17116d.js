...
ref)
{let _allow = allow,_sandbox = sandbox,_title = title;
  if (!includes(_allow, BLOCK_SYNC_XHR)) {
    throw new Error(
    `'allow' prop must contain "${BLOCK_SYNC_XHR}". Found "${_allow}".`);
  ...
  const { 0: { name, src }, 1: setNameAndSrc } = useState({ name: nameProp, src: srcProp });
  useLayoutEffect(() => {
  ...
  return (
    Preact.createElement(IframeEmbed, {
      allow: _allow,
      ...
      ref: iframeRef,
      ready: !!name,
      sandbox: excludeSandbox ? undefined : _sandbox,
      src: src,
      title: _title, ...
      rest }));
