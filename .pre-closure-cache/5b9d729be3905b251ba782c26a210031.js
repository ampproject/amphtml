 ...
 */
export function fetchPolyfill(input, init = {}) {let _init = init;
  return new Promise(function (resolve, reject) {
    const requestMethod = normalizeMethod(_init.method || 'GET');
    const xhr = createXhrRequest(requestMethod, input);

    if (_init.credentials == 'include') {
      xhr.withCredentials = true;
    }

    if (_init.responseType in allowedFetchTypes) {
      xhr.responseType = _init.responseType;
    }

    if (_init.headers) {
      Object.keys(_init.headers).forEach(function (header) {
        xhr.setRequestHeader(header, _init.headers[header]);
      });
    }
...
export class Response extends FetchResponse {
   ...
   */
  constructor(body, init = {}) {let _init2 = init;
    ...
    data.status = _init2.status === undefined ? 200 : parseInt(_init2.status, 10);

    if (isArray(_init2.headers)) {
      /** @type {!Array} */(_init2.headers).forEach((entry) => {
        const headerName = entry[0];
        const headerValue = entry[1];
        lowercasedHeaders[String(headerName).toLowerCase()] =
        String(headerValue);
      });
    } else if (isObject(_init2.headers)) {
      for (const key in _init2.headers) {
    ...
    if (_init2.statusText) {
      data.statusText = String(_init2.statusText);
    }
