...
isAllowlistedProp } from "./web-animation-types";
...
export class Builder {
   ...
   */
  constructor(win, rootNode, baseUrl, vsync, owners, options = {}) {let _options = options;
    /** @const @private */
    this.win_ = win;

    /** @const @private */
    this.css_ = new CssContextImpl(win, rootNode, baseUrl, _options);
   ...
   */
  createRunner(spec, opt_args, opt_positionObserverData = null) {let _opt_positionObserverData = opt_positionObserverData;
    return this.resolveRequests([], spec, opt_args).then((requests) => {
      ...
      return Promise.all(this.loaders_).then(() => {
        return this.isAnimationWorkletSupported_() && _opt_positionObserverData ?
        new ScrollTimelineWorkletRunner(
        this.win_,
        requests,
        _opt_positionObserverData) :
  ...
  vars = null,
  timing = null)
  {let _target = target,_index = index,_vars = vars,_timing = timing;
    const scanner = this.createScanner_(path, _target, _index, _vars, _timing);
...
export class MeasureScanner extends Scanner {
  ...
  mergeTiming_(newTiming, prevTiming) {
    ...
    newTiming.iterations, /** @type {number} */(
    prevTiming.iterations));
    ...
    userAssert(
    fill === "none" || fill === "forwards" || fill === "backwards" || fill === "both" || fill === "auto",
    `Unknown fill: ${fill}`);
...
class CssContextImpl {
  ...
  getCurrentIndex() {
    this.requireTarget_();
    return (/** @type {number} */(this.currentIndex_));
  }

  /** @override */
  getTargetLength() {
    this.requireTarget_();
    return (/** @type {number} */(this.targetLength_));
  }
