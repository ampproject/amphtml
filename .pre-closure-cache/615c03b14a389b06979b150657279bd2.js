...
export class Navigation {
   ...
   * }=} options
   */
  navigateTo(win, url, opt_requestedBy, options = {}) {let _options = options;
    const { opener = false, target = '_top' } = _options;
  ...
  scrollToElement_(elem, hash) {
    ...
    if (elem) {
      ...
      () => this.viewport_. /*OK*/scrollIntoView( /** @type {!Element} */(elem)),
      1);
...
function maybeExpandUrlParams(ampdoc, e) {
  const target = closestAncestorElementBySelector( /** @type {!Element} */(
  e.target),
  'A');
