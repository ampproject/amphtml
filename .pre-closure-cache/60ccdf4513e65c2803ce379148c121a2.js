...
export class TimeBasedAdvancement extends AdvancementConfig {
  ...
  /** @override */
  stop(canResume = false) {let _canResume = canResume;
    super.stop();
    ...
    // paused.
    this.remainingDelayMs_ = _canResume ?
...
export class MediaBasedAdvancement extends AdvancementConfig {
  ...
  start() {
    ...
    (this.element_.build ? this.element_.build() : _resolvedPromise()).then(() =>
  ...
  startHtmlMediaElement_() {
    const mediaElement = /** @type {!Element} */(
    this.mediaElement_);
