...
export class PreactBaseElement extends AMP.BaseElement {
   ...
   * @protected
   */
  registerApiAction(alias, handler, minTrust = ActionTrust.DEFAULT) {let _minTrust = minTrust;
    this.registerAction(
    alias,
  ...
  rerender_() {
    ...
    for (let i = 0; i < useContexts.length; i++) {
      ...
      if (value) {
        comp = Preact.createElement(Context.Provider, { value: value }, comp);
      }
    }

    // Add AmpContext with renderable/playable proeprties.
    const v = Preact.createElement(WithAmpContext, { ...this.context_ }, comp);
