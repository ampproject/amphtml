import { resolvedPromise as _resolvedPromise } from "./../../../src/core/data-structures/promise";const _template = ["<i-amphtml-poster></i-amphtml-poster>"]; /**
...
export class AmpVideo extends AMP.BaseElement {
   ...
   * @private
   */
  createSourceElement_(src, type, attributes = {}) {let _attributes = attributes;
    const { element } = this;
    ...
    addAttributesToElement(source, _attributes);
    return source;
  }
  ...
  installEventHandlers_() {
    const video = /** @type {!Element} */(this.video_);
  ...
  createPosterForAndroidBug_() {
    ...
    const poster = htmlFor(element)(_template);
   ...
   */
  fullscreenEnter() {
    fullscreenEnter( /** @type {!Element} */(this.video_));
  }
   ...
   */
  fullscreenExit() {
    fullscreenExit( /** @type {!Element} */(this.video_));
  }
