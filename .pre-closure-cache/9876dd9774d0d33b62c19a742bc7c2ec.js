const _template = ["<button aria-label=\"Unmute video\" class=\"i-amphtml-video-mask i-amphtml-fill-content\" tabindex=0></button>"],_template2 = ["<i-amphtml-video-icon class=amp-video-eq><div class=amp-video-eq-col><div class=amp-video-eq-filler></div><div class=amp-video-eq-filler></div></div></i-amphtml-video-icon>"]; /**
...
export function renderIcon(win, elOrDoc) {
  ...
  const firstCol = /** @type {!Element} */(icon.firstElementChild);
  for (let i = 0; i < 4; i++) {
