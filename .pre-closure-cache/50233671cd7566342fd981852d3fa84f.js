...
class DefaultTransportSerializer {
  /** @override */
  generateRequest(baseUrl, segment, withPayload = false) {let _withPayload = withPayload;
    if (_withPayload) {
      return {
  ...
  /** @override */
  generateBatchRequest(baseUrl, segments, withPayload = false) {let _withPayload2 = withPayload;
    if (_withPayload2) {
      return {
        url: baseUrl.replace(EXTRA_URL_PARAM_VAR, ''),
