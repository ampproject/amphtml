...
ref)
{let _matchesMessagingOrigin = matchesMessagingOrigin,_ready = ready;
  ...
  useLayoutEffect(() => {
    ...
    const handler = (event) => {
      ...
      !_matchesMessagingOrigin(event.origin))
      {
        return;
      }
  ...
  }, [_matchesMessagingOrigin, messageHandler, mount, _ready]);

  return (
    Preact.createElement(ContainWrapper, { ...rest, layout: true, size: true, paint: true },
    mount && _ready &&
