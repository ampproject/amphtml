...
export class Extensions {
   ...
   * @return {!Promise<!ExtensionDef>}
   */
  preloadExtension(extensionId, version = DEFAULT_VERSION) {let _version = version;
    if (extensionId == 'amp-embed') {
      extensionId = 'amp-ad';
    }
    const holder = this.getExtensionHolder_(extensionId, _version);
    this.insertExtensionScriptIfNeeded_(extensionId, _version, holder);
   ...
   * @return {!Promise<!ExtensionDef>}
   */
  installExtensionForDoc(ampdoc, extensionId, version = DEFAULT_VERSION) {let _version2 = version;
    ...
    ampdoc.declareExtension(extensionId, _version2);
    stubElementIfNotKnown(ampdoc.win, extensionId);
    return (extLoaders[extensionId] = this.preloadExtension(
    extensionId,
    _version2).
    then(() => this.installExtensionInDoc(ampdoc, extensionId, _version2)));
  }
   ...
   * @return {!Promise}
   */
  importUnwrapped(win, extensionId, version = DEFAULT_VERSION, latest = true) {let _version3 = version,_latest = latest;
    ...
    if (scriptElement) {
      ...
      scriptElement = createExtensionScript(this.win, extensionId, _version3);
   ...
   */
  loadElementClass(elementName, version = DEFAULT_VERSION) {let _version4 = version;
    return this.preloadExtension(elementName, _version4).then((extension) => {
  ...
  addDocFactory(factory, opt_forName) {
    ...
    if (this.currentExtensionId_ && this.ampdocService_.isSingleDoc()) {
      ...
      const extensionId = /** @type {string} */(this.currentExtensionId_);
      const version = /** @type {string} */(this.currentExtensionVersion_);
   ...
   * @return {!Promise}
   */
  installExtensionInDoc(ampdoc, extensionId, version = DEFAULT_VERSION) {let _version5 = version;
    ampdoc.declareExtension(extensionId, _version5);
    return this.waitFor_(this.getExtensionHolder_(extensionId, _version5)).then(
    () => {
      const holder = this.getExtensionHolder_(extensionId, _version5);
