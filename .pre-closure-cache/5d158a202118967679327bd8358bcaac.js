...
SubscriptionAnalyticsEvents } from "./analytics";
...
PageConfigResolver } from "../../../third_party/subscriptions-project/config";
...
export class SubscriptionService {
  ...
  getReaderId(platformKey) {
    ...
    if (!readerIdPromise) {
      const consent = _resolvedPromise();
   ...
   * @private
   */
  startAuthorizationFlow_(shouldActivatePlatform = true) {let _shouldActivatePlatform = shouldActivatePlatform;
    ...
    return promises.then((results) => {
      ...
      this.handleGrantState_({ granted, shouldActivatePlatform: _shouldActivatePlatform });
   ...
   * @return {!Promise<boolean>}
   */
  delegateActionToService(action, platformKey, sourceId = null) {let _sourceId = sourceId;
    return new Promise((resolve) => {
      this.platformStore_.onPlatformResolves(platformKey, (platform) => {
        ...
        resolve(platform.executeAction(action, _sourceId));
      });
    });
  }
