...
applySandbox,
generateSentinel,
getDefaultBootstrapBaseUrl } from "../../../src/3p-frame";
...
createSecureFrame,
isAttributionReportingSupported } from "./secure-frame";
...
incrementLoadingAds,
is3pThrottled } from "../../amp-ad/0.1/concurrent-load";
...
isSrcdocSupported,
preloadFriendlyIframeEmbedExtensions } from "../../../src/friendly-iframe-embed";
...
inThis = undefined,
onError = undefined)
{let _inThis = inThis,_onError = onError;
  return (...fnArgs) => {
    try {
      return fn.apply(_inThis, fnArgs);
    } catch (err) {
      if (_onError) {
        try {
          ...
          fnArgs.unshift(err);
          return _onError.apply(_inThis, fnArgs);
        } catch (captureErr) {
...
export class AmpA4A extends AMP.BaseElement {
  ...
  */
   ...
   * @protected
   */
  destroyFrame(force = false) {let _force = force;
    if (!_force && this.isRefreshing) {
      return;
    }
  ...
  getNonAmpCreativeRenderingMethod(headerValue) {
    if (headerValue) {
      if (!(headerValue === "client_cache" || headerValue === "safeframe" || headerValue === "nameframe" || headerValue === "iframe_get")) {
        dev().error(
        'AMP-A4A',
