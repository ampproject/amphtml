...
XORIGIN_MODE,
assignAdUrlToError } from "../../amp-a4a/0.1/amp-a4a";
...
truncAndTimeUrl } from "../../../ads/google/a4a/utils";
...
getFlexibleAdSlotData } from "./flexible-ad-slot-utils";
...
serializeTargeting,
sraBlockCallbackHandler } from "./sra-utils";
...
incrementLoadingAds,
is3pThrottled,
waitFor3pThrottle } from "../../amp-ad/0.1/concurrent-load";
...
metaJsonCreativeGrouper } from "../../../ads/google/a4a/line-delimited-response-handler";
...
export class AmpAdNetworkDoubleclickImpl extends AmpA4A {
  ...
  expandValue_(value) {
    ...
    if (isArray(value)) {
      return Promise.all(
      value.map((arrVal) => this.expandString_( /** @type {string} */(arrVal))));

    }
    return this.expandString_( /** @type {string} */(value));
  }
  ...
  extractSize(responseHeaders) {
    ...
    this.troubleshootData_.creativeId = /** @type {string} */(
    responseHeaders.get('google-creative-id') || '-1');

    this.troubleshootData_.lineItemId = /** @type {string} */(
    ...
    if (responseHeaders.get('amp-ff-pageview-tokens')) {
      ...
      this.setPageviewStateToken( /** @type {string} */(
  ...
  /** @override */
  getAdditionalContextMetadata(isSafeFrame = false) {let _isSafeFrame = isSafeFrame;
    if (!this.isFluidRequest_ && !_isSafeFrame) {
      return;
    }
    const creativeSize = this.getCreativeSize();
    devAssert(creativeSize);
    if (this.isRefreshing) {
      if (this.safeframeApi_) {
        this.safeframeApi_.destroy();
      }
  ...
  postTroubleshootMessage() {
    ...
    devAssert(this.troubleshootData_.adUrl);
...
export function getNetworkId(element) {
  const networkId = /^(?:\/)?(\d+)/.exec( /** @type {string} */(
