const _template = ["<svg viewBox=\"0 0 40 40\"><defs><linearGradient id=grad><stop stop-color=\"rgb(105, 105, 105)\"></stop><stop offset=100% stop-color=\"rgb(105, 105, 105)\" stop-opacity=0></stop></linearGradient></defs><path d=\"M11,4.4 A18,18, 0,1,0, 38,20\" stroke=url(#grad)></path></svg>"]; /**
...
import {
isAmpElement,
whenUpgradedToCustomElement } from "../../../src/amp-element-helpers";

import { setImportantStyles, setStyles, toggle } from "../../../src/core/dom/style";
...
export class ConsentUI {
  ...
  init_(config, opt_postPromptUI) {
    if (opt_postPromptUI) {
      ...
      this.ui_ = /** @type {!Element} */(postPromptUI);
    ...
    if (promptUI) {
      ...
      this.ui_ = /** @type {!Element} */(promptElement);
    } else if (promptUISrc) {
  ...
  show(isActionPromptTrigger) {
    ...
    toggle( /** @type {!Element} */(this.parent_), true);
  ...
  hide() {
    ...
    this.baseInstance_.mutateElement(() => {
      ...
      toggle( /** @type {!Element} */(this.ui_), false);
      this.isVisible_ = false;
  ...
  createPlaceholder_() {
    ...
    const loadingSpinner = htmlFor(placeholder)(_template);
  ...
  loadIframe_() {
    ...
    if (!elementByTag(this.parent_, 'placeholder')) {
      insertAtStart(this.parent_, /** @type {!Element} */(this.placeholder_));
    }
    ...
    const iframePromise = this.promptUISrcPromise_.then((expandedSrc) => {
      ...
      return this.getClientInfoPromise_().then((clientInfo) => {
        ...
        insertAtStart(this.parent_, /** @type {!Element} */(this.ui_));
      });
    });
    ...
    this.baseInstance_.mutateElement(() => {
      toggle( /** @type {!Element} */(this.placeholder_), true);
    })]);
  ...
  showIframe_() {
    ...
    toggle( /** @type {!Element} */(this.placeholder_), false);
    toggle( /** @type {!Element} */(this.ui_), true);
    if (this.modalEnabled_) {
      classList.add(consentUiClasses.modal);
      tryFocus( /** @type {!Element} */(this.ui_));
    }
  ...
  resetIframe_() {
    ...
    this.win_.setTimeout(() => {
      if (this.removeIframe_) {
        removeElement( /** @type {!Element} */(this.ui_));
      }
   ...
   * */
  sendEnterFullscreenResponse_(requestType, requestAction, isError = false) {let _isError = isError;
    this.sendIframeMessage_(
    ampConsentMessageType.response,
    requestType,
    requestAction,
    _isError ? actionState.error : actionState.success,
    _isError ? FULLSCREEN_ERROR : FULLSCREEN_SUCCESS);
