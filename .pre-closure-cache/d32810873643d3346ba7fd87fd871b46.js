...
export class StoryAnalyticsService {
   ...
   * @param {Element=} element
   */
  triggerEvent(eventType, element = null) {let _element = element;
    this.incrementPageEventCount_(eventType);

    triggerAnalyticsEvent(
    this.element_,
    eventType,
    this.updateDetails(eventType, _element));
   ...
   * @return {!JsonObject}}
   */
  updateDetails(eventType, element = null) {let _element2 = element;
    const details = {};
    ...
    if (_element2) {
      details.tagName =
      _element2[ANALYTICS_TAG_NAME] || _element2.tagName.toLowerCase();
      Object.assign(
      vars,
