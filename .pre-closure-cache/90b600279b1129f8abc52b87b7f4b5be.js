...
export class ResourcesImpl {
  ...
  checkForDupes = false,
  ignoreQuota = false)
  {let _checkForDupes = checkForDupes,_ignoreQuota = ignoreQuota;
    ...
    if (this.documentReady_) {
      ...
      this.buildResourceUnsafe_(resource, _ignoreQuota);
    } else if (!resource.isBuilt() && !resource.isBuilding()) {
      if (!_checkForDupes || !this.pendingBuildResources_.includes(resource)) {
   ...
   * @private
   */
  buildResourceUnsafe_(resource, ignoreQuota = false) {let _ignoreQuota2 = ignoreQuota;
    if (
    !_ignoreQuota2 &&
  ...
  scrolled_(event) {
    ...
    const scrolled = /** @type {!Element} */(target);
