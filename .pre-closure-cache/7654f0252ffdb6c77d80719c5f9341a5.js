...
{let _loadingProp = loadingProp,_playableProp = playableProp,_renderableProp = renderableProp;
  const parent = useAmpContext();
  const renderable = _renderableProp && parent.renderable;
  const playable = renderable && _playableProp && parent.playable;
  const loading = loadingReducer(
  renderable ? Loading.AUTO : Loading.LAZY,
  loadingReducer(_loadingProp, parent.loading));
