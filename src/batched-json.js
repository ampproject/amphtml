...
export function requestForBatchFetch(ampdoc, element, replacement, refresh) {
  ...
  return promise.then(xhrUrl => {
    ...
    if (replacement == UrlReplacementPolicy.OPT_IN) {
      ...
      if (invalid.length > 0) {
        throw user().createError('URL variable substitutions in CORS fetches from dynamic URLs (e.g. via amp-bind) require opt-in. Please add data-amp-replace="%s" to the <%s> element. See https://bit.ly/amp-var-subs.', invalid.join(' '),element.tagName);
      }
    }
    const fetchOpt = {};
