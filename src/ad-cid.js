...
export function getAdCid(adElement) {
  ...
  return getClientAdCid(adElement.win, adElement.getAmpDoc(),
    config.clientIdScope, config.clientIdCookieName);
}

/**
 * @param {!Window} win
 * @param {!Node|!./service/ampdoc-impl.AmpDoc} doc
 ...
 *     - `amp-analytics` which provides the CID service was not installed.
 */
export function getClientAdCid(
    win, doc, clientIdScope, opt_clientIdCookieName) {
  dev().assert(clientIdScope);
