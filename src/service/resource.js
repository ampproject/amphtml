 ...
 * @enum {number}
 */
export const RESOURCE_STATE_ENUM = {
  /**
...
export class Resource {
  ...
  constructor(id, element, resources) {
    ...
    if (this.state_ == RESOURCE_STATE_ENUM.NOT_BUILT && element.isBuilding()) {
      this.build();
    }
  ...
  build() {
    ...
    return this.element.buildInternal().then(
      () => {
        this.isBuilding_ = false;
        this.state_ = RESOURCE_STATE_ENUM.NOT_LAID_OUT;
        // TODO(dvoytenko): merge with the standard BUILT signal.
  ...
  measure() {
    ...
    if (
      ...
      if (this.element.isUpgraded()) {
        if (this.state_ == RESOURCE_STATE_ENUM.NOT_LAID_OUT) {
          // If the element isn't laid out yet, then we're now ready for layout.
          this.state_ = RESOURCE_STATE_ENUM.READY_FOR_LAYOUT;
        } else if (
          (this.state_ == RESOURCE_STATE_ENUM.LAYOUT_COMPLETE ||
            this.state_ == RESOURCE_STATE_ENUM.LAYOUT_FAILED) &&
  ...
  layoutScheduled(scheduleTime) {
    this.state_ = RESOURCE_STATE_ENUM.LAYOUT_SCHEDULED;
  ...
  startLayout() {
    ...
    if (this.state_ == RESOURCE_STATE_ENUM.LAYOUT_COMPLETE) {
      return Promise.resolve();
    }
    if (this.state_ == RESOURCE_STATE_ENUM.LAYOUT_FAILED) {
      return Promise.reject(this.lastLayoutError_);
    }

    devAssert(
      this.state_ != RESOURCE_STATE_ENUM.NOT_BUILT,
    ...
    if (this.state_ != RESOURCE_STATE_ENUM.LAYOUT_SCHEDULED) {
  ...
  layoutComplete_(success, signal, opt_reason) {
    ...
    this.layoutPromise_ = null;
    this.state_ = success
      ? RESOURCE_STATE_ENUM.LAYOUT_COMPLETE
