...
The `bento-lightbox-gallery` component provides a "lightbox" experience for AMP
...
To use `bento-lightbox-gallery`, ensure the required script is included in your
...
[`<amp-lightbox>`](../bento-lightbox/bento-lightbox.md).
...
its children. Currently, the `<bento-lightbox-gallery>` component only supports
...
You can style the caption of a `bento-lightbox-gallery` component
by targeting the `bento-lightbox-gallery-caption` class with standard CSS.
...
specify the image `id`: `on="tap:bento-lightbox-gallery.open(id='image-id')"`.

## Analytics

To track usage of `bento-lightbox-gallery`, please use one of the following
analytics events:
...
To find the standalone version of `bento-lightbox-gallery`, see [**`bento-lightbox-gallery`**](./1.0/README.md).

## Validation

See [`amp-image-lightbox-gallery` rules](../../../../extensions/amp-lightbox-gallery/validator-amp-lightbox-gallery.protoascii)
