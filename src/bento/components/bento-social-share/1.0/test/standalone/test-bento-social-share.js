import {CSS} from '#build/bento-social-share-1.0.css';

import {BaseElement as BentoSocialShare} from '#bento/components/bento-social-share/1.0/base-element';
...
describes.realWin(
  'bento-social-share-v1.0',
  {
    amp: {
      runtimeOn: true,
      extensions: ['bento-social-share:1.0'],
  ...
  (env) => {
    ...
    beforeEach(() => {
      win = env.win;
      defineBentoElement('bento-social-share', BentoSocialShare, win);
      adoptStyles(win, CSS);
      doc = win.document;
      doc.title = 'Test Title';
    });

    it.only('renders custom endpoint when not using a pre-configured type', async () => {
      element = win.document.createElement('bento-social-share');
    ...
    it('renders children and hides default icon when children are provided', async () => {
      ...
      // when a child is included in the bento-social-share element
      expect(
    ...
    describe('dynamically update attributes', () => {
      it('updates default url and css class when "type" attribute is updated', async () => {
        ...
        expect(element.className).to.include('bento-social-share-email');
        expect(element.className).to.not.include('bento-social-share-facebook');
        ...
        expect(element.className).to.include('bento-social-share-facebook');
        expect(element.className).to.not.include('bento-social-share-email');
      });
