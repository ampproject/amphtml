...
function getTemplateContentsAsString(template) {
  ...
  for (let i = 0, count = template.content.childElementCount; i < count; i++) {
 ...
 * @return {!Object}
 */
export function makeTemplateObject(template) {
  const contents = getTemplateContentsAsString(template)
    .replaceAll(regex, '')
    .split('$');

  return Object.defineProperty(contents, 'raw', {
    value: contents.slice(0),
  });
  // render(html(obj, data), document.getElementById('app'));
}
