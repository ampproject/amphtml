...
class AmpWorker {
  ...
  receiveMessage_(event) {
    ...
    if (!message) {
      dev().error(TAG, 'Received unexpected message (%s, %s) from worker.', method,id);
      return;
    }
    dev().assert(method == message.method, 'Received mismatched method (%s, %s), expected %s.', method,id,message.method);
