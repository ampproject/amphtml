...
import {Layout, applyStaticLayout, isInternalElement} from '../layout';
import {LayoutDelayMeter} from '../layout-delay-meter';
import {LoaderProp, blockRender} from './loader';
...
function createBaseCustomElementClass(win) {
  ...
  class BaseCustomElement extends htmlElement {
    ...
    completeUpgrade_(newImpl, upgradeStartTime) {
      ...
      // Block rendering of this component while it's DOM is parsing. This
      // is technically not necessary for Bento components, but still needed
      // for older components. And even for Bento components this has some
      // performance benefits.
      blockRender(this);

      // By default, only build when the element can be rendered at all.
      subscribe(this, [CanRender], (canRender) => {
        if (canRender) {
          this.build();
        }
      });
    ...
    connectedCallback() {
      ...
      if (this.everAttached) {
        ...
        // Indicate that the element is ready to be loaded. When it should be
        // loaded is decided by the loader.
        setProp(this, LoadPendingState, this, true);
        setProp(this, LoadedState, this, false);
    ...
    layoutCallback() {
      ...
      return promise.then(
        () => {
          setProp(this, LoadPendingState, this, false);
          setProp(this, LoadedState, this, true);
...
function isPlaceholder(element) {
  // QQQ: additional checks such as !input/textarea.
