...
import {updatePackages, updateSubpackages} from '../common/update-packages.mjs';
import {cyan, green, magenta, red} from 'kleur/colors';
...
function getTaskSourceFilePath(taskSourceFileName) {
  ...
  const taskSourceFilePath = path.join(tasksDir, taskSourceFileName + '.mjs');
  console.log('getTaskSourceFilePath', {
    tasksDir,
    taskSourceFileName,
    taskSourceFilePath,
  });
 ...
 * @param {string} taskSourceFileName
 * @param {string} taskFuncName
 * @return {Promise<TaskFuncDef>}
 */
async function getTaskFunc(taskSourceFileName, taskFuncName) {
  const taskSourceFilePath = getTaskSourceFilePath(taskSourceFileName);
  console.log({taskSourceFilePath});
  const taskFunc = (await import(taskSourceFilePath))[taskFuncName]; //require(taskSourceFilePath)[taskFuncName];
...
export async function createTask(
  ...
  if (isInvokedTask || isDefaultTask) {
    ...
    const taskFunc = await getTaskFunc(taskSourceFileName, taskFuncName);
