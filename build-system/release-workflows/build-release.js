...
runReleaseJob(jobName, async () => {
  ...
  timedExecOrThrow(`amp release --${ESM} --dedup_v0`);
  fs.ensureDirSync(`/tmp/workspace/releases/${ESM}`);
  fs.copySync('release/', `/tmp/workspace/releases/${ESM}`);
});
