...
import {checkForUnknownDeps} from './check-for-unknown-deps.mjs';
import {CLOSURE_SRC_GLOBS} from './sources.mjs';
import {cpus} from 'os';
import {cyan, green} from 'kleur/colors';
import {log, logLocalDev} from '../common/logging.mjs';
import {postClosureBabel} from './post-closure-babel.mjs';
import {preClosureBabel} from './pre-closure-babel.mjs';
import {runClosure} from './closure-compile.mjs';
import {sanitize} from './sanitize.mjs';
import {VERSION as internalRuntimeVersion} from './internal-version.mjs';
import {writeSourcemaps} from './helpers.mjs';
 ...
 * @return {Promise<void>}
 */
export async function closureCompile(
  entryModuleFilename,
  outputDir,
  outputFilename,
  options,
...
async function compile(
  ...
  const srcFiles = globby.sync(srcs);
  const transformedSrcFiles = [];
  for (const src of srcFiles) {
    const transformed = await preClosureBabel(src, outputFilename, options);
    transformedSrcFiles.push(transformed);
  }
 ...
 */
export function printClosureConcurrency() {
  log(
    green('Using up to'),
