...
import {none as noneWrapper} from '../compile/compile-wrappers.mjs';
import {VERSION as internalRuntimeVersion} from '../compile/internal-version.mjs';
import {applyConfig, removeConfig} from './prepend-global/index.mjs';
import {closureCompile} from '../compile/compile.mjs';
import {cyan, green, red} from 'kleur/colors';
import {getEsbuildBabelPlugin} from '../common/esbuild-babel.mjs';
import {importJSON} from '../common/import-json.mjs';
import {isCiBuild} from '../common/ci.mjs';
import {jsBundles} from '../compile/bundles.config.mjs';
import {log, logLocalDev} from '../common/logging.mjs';
import {minify as terserMinify} from 'terser';
import {thirdPartyFrames} from '../test-configs/config.mjs';
import {watch} from 'chokidar';

const argv = minimist(process.argv.slice(2));
/** @type {Object} */
const experimentDefines = importJSON(
  import.meta.url,
  '../global-configs/experiments-const.json'
);
 ...
 */
export const MINIFIED_TARGETS = ['alp', 'amp4ads-v0', 'shadow-v0', 'v0'];
 ...
 * @return {!Promise}
 */
export function doBuildJs(jsBundles, name, extraOptions) {
 ...
 * @return {Promise<void>}
 */
export async function bootstrapThirdPartyFrames(options) {
 ...
 * @return {Promise<void>}
 */
export async function compileCoreRuntime(options) {
 ...
 * @return {!Promise}
 */
export async function compileAllJs(options) {
 ...
 * @return {string}
 */
export function maybeToEsmName(name) {
 ...
 * @return {string}
 */
export function maybeToNpmEsmName(name) {
 ...
 * @return {!Promise}
 */
export async function esbuildCompile(srcDir, srcFilename, destDir, options) {
  const startTime = Date.now();
  const entryPoint = path.join(srcDir, srcFilename);
  ...
  function splitWrapper() {
    const wrapper = options.wrapper || noneWrapper;
...
async function minify(code, map) {
  ...
  const minified = await terserMinify(code, terserOptions);
  if (!minified.code) {
    throw new Error(
 ...
 * @return {!Promise}
 */
export async function compileJs(srcDir, srcFilename, destDir, options) {
  options = options || {};
  const entryPoint = path.join(srcDir, srcFilename);
 ...
 */
export function endBuildStep(stepName, targetName, startTime) {
 ...
 */
export function printConfigHelp(command) {
  log(
 ...
 */
export function printNobuildHelp() {
 ...
 * @return {!Promise}
 */
export async function maybePrintCoverageMessage(covPath = '') {
 ...
 * @return {!Promise}
 */
export async function applyAmpConfig(targetFile, localDev, fortesting) {
