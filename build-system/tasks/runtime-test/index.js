...
async function runTests() {
  ...
  if (argv.saucelabs && !argv.integration) {
    log(
      red('ERROR:'),
      'Only integration tests may be run on the full set of',
      'Sauce Labs browsers'
    );
  ...
  if (argv.files) {
    ...
    c.files = c.files.concat(
      config.commonIntegrationTestPaths,
      config.integrationTestPaths
    );
  } else if (argv.unit) {
    if (argv.saucelabs_lite) {
      c.files = c.files.concat(
        config.commonUnitTestPaths,
        config.unitTestOnSaucePaths
      );
    } else {
      c.files = c.files.concat(
        config.commonUnitTestPaths,
        config.unitTestPaths
      );
    }
  } else if (argv.a4a) {
  ...
  c.client.amp = {
    ...
    testOnIe:
      !!argv.ie || (!!argv.saucelabs && saucelabsBrowsers.includes('SL_IE_11')),
  };
  ...
  const server = gulp.src(process.cwd(), {base: '.'}).pipe(
    webserver({
      ...
      log(
        yellow(
          'Shutting down test responses server on ' +
  ...
  log(
    yellow(
      'Started test responses server on ' +
        `localhost:${karmaDefault.client.testServerPort}`
    )
  );
...
test.flags = {
  ...
  'local-changes':
    '  Run unit tests directly affected by the files ' +
    'changed in the local branch',
};
