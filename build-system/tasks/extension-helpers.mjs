...
import * as wrappers from '../compile/compile-wrappers.mjs';
import {
  compileJs,
  doBuildJs,
  endBuildStep,
...
} from '../compile/bundles.config.mjs';
import {VERSION as internalRuntimeVersion} from '../compile/internal-version.mjs';
import {analyticsVendorConfigs} from './analytics-vendor-configs.mjs';
import {compileJison} from './compile-jison.mjs';
import {cyan, green, red} from 'kleur/colors';
import {isCiBuild} from '../common/ci.mjs';
import {jsifyCssAsync} from './css/jsify-css.mjs';
import {jssOptions} from '../babel-config/jss-config.mjs';
import {log} from '../common/logging.mjs';
import {TransformCache, batchedRead} from '../common/transform-cache.mjs';
 ...
 * @param {boolean=} includeLatest
 */
export function maybeInitializeExtensions(
 ...
 */
export function setExtensionsToBuildFromDocuments(examples) {
 ...
 * @return {!Array<string>}
 */
export function getExtensionsToBuild(preBuild = false) {
 ...
 * @param {boolean=} preBuild
 */
export function parseExtensionFlags(preBuild = false) {
  if (isCiBuild()) {
    return;
  }
 ...
 * @return {!Promise<void>}
 */
export async function doBuildExtension(extensions, extension, options) {
