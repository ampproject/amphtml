...
| `gulp visual-diff`                                        | Runs all visual diff tests on a headless instance of local Chrome after building the unminified runtime with the `prod` version of `AMP_CONFIG`. Requires `PERCY_TOKEN` to be set as an environment variable or passed to the task with `--percy_token`. |
| `gulp visual-diff --compiled`                             | Same as above, but builds the minified runtime.                                                                                                                                                                                                          |
...
First, build the AMP runtime in minified or unminified mode:

```sh
gulp build --fortesting # unminified mode
gulp dist --fortesting # minified mode
```

Next, run the `gulp` task that invokes the visual diff tests:

```sh
gulp visual-diff --nobuild # unminified mode
gulp visual-diff --compiled --nobuild # minified mode
```

Note that if you drop the `--nobuild` flag, `gulp visual-diff` will run `gulp dist|build --fortesting` on each execution.
