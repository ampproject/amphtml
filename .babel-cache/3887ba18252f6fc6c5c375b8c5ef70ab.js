...
function getMode_(win) {
  var hashQuery = parseQueryString(
...
function getRtvVersion(win) {
  ...
  if (!rtvVersion || coreMode.isTest(win)) {var _win$AMP_CONFIG;
    // Currently `internalRuntimeVersion` and thus `mode.version` contain only
    // major version. The full version however must also carry the minor version.
    // We will default to production default `01` minor version for now.
    // TODO(erwinmombay): decide whether internalRuntimeVersion should contain
    // minor version.
    rtvVersion = (((_win$AMP_CONFIG = win.AMP_CONFIG) === null || _win$AMP_CONFIG === void 0) ? (void 0) : _win$AMP_CONFIG.v) || "01".concat(coreMode.version());
  }
  return rtvVersion;
}
