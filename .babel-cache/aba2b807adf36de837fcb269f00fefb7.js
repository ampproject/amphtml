...
getRequiredSandboxFlags } from "./core/3p-frame";
...
{var options = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : {};
  var _options$allowFullscr = options.allowFullscreen,allowFullscreen = _options$allowFullscr === void 0 ? false : _options$allowFullscr,initialIntersection = options.initialIntersection;
  ...
  var attributes = getFrameAttributes(
  parentWindow,
  parentElement,
  opt_type,
  opt_context);
...
export function getBootstrapUrl(type) {
  ...
  if (getMode().localDev || getMode().test) {
    var filename = mode.isMinified() ? "./vendor/".concat(
    type) : "./vendor/".concat(
    type, ".max");
  ...
  internalRuntimeVersion(), "/vendor/").concat(type).concat(extension);
}
...
export function getDefaultBootstrapBaseUrl(parentWindow, opt_srcFileBasename) {
  ...
  return (
  'https://' +
  parentWindow.__AMP_DEFAULT_BOOTSTRAP_SUBDOMAIN + ".".concat(
  urls.thirdPartyFrameHost, "/").concat(internalRuntimeVersion(), "/") + "".concat(
...
export function getDevelopmentBootstrapBaseUrl(parentWindow, srcFileBasename) {
  ...
  '/dist.3p/' + (
  mode.isMinified() ? "".concat(
  internalRuntimeVersion(), "/").concat(srcFileBasename) : "current/".concat(
  srcFileBasename, ".max")) +
  '.html');
...
function getAdsLocalhost(win) {
  var adsUrl = urls.thirdParty; // local dev with a non-localhost server
...
opt_strictForUnitTest)
{
  ...
  return "".concat(url, "?").concat(internalRuntimeVersion());
}
