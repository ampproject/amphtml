var _template = ["<iframe frameborder=0 allowfullscreen></iframe>"]; /**
...
export function redispatch(element, event, events) {
  ...
  (isArray(dispatchEvent) ? dispatchEvent : [dispatchEvent]).forEach(function (e) {
    dispatchCustomEvent(element, /** @type {string} */(e));
  });
  return true;
}
 ...
 * @param {*} message
 */
export function postMessageWhenAvailable(iframe, message) {var _iframe$contentWindow;
  (iframe === null || iframe === void 0) ? (void 0) : ((_iframe$contentWindow = iframe.contentWindow) === null || _iframe$contentWindow === void 0) ? (void 0) : _iframe$contentWindow. /*OK*/postMessage(message, '*');
}
