...
export var Navigation = /*#__PURE__*/function () {
   ...
   * @param {!./ampdoc-impl.AmpDoc} ampdoc
   */
  function Navigation(ampdoc) {var _this = this;_classCallCheck(this, Navigation);
   ...
   * @param {!./ampdoc-impl.AmpDoc} ampdoc
   * @param {!Window} win
   */_createClass(Navigation, [{ key: "cleanup", value:
    ...
    function openWindow(win, url, target, opener) {
      var options = '';
     ...
     * }=} options
     */ }, { key: "navigateTo", value:
    function navigateTo(win, url, opt_requestedBy) {var options = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : {};
      var _options$opener = options.opener,opener = _options$opener === void 0 ? false : _options$opener,_options$target = options.target,target = _options$target === void 0 ? '_top' : _options$target;
      ...
      userAssert(
      VALID_TARGETS.includes(target), "Target '".concat(
     ...
     */ }, { key: "navigateToAmpUrl", value:
    function navigateToAmpUrl(url, requestedBy) {
     ...
     * @return {!Array<string>}
     * @private
     */ }, { key: "queryA2AFeatures_", value:
    function queryA2AFeatures_() {
      var meta = this.rootNode_.querySelector(
    ...
    function handleClick_(element, e) {
     ...
     * @param {!Event} e
     * @private
     */ }, { key: "handleContextMenuClick_", value:
    function handleContextMenuClick_(element, e) {
     ...
     * @param {!Element} element
     * @param {!Event} e
     */ }, { key: "applyAnchorMutators_", value:
    function applyAnchorMutators_(element, e) {
     ...
     * @param {string} url
     * @return {string}
     */ }, { key: "applyNavigateToMutators_", value:
    function applyNavigateToMutators_(url) {
     ...
     * @param {!Element} el
     * @private
     */ }, { key: "expandVarsForAnchor_", value:
    function expandVarsForAnchor_(el) {
     ...
     * @return {boolean}
     * @private
     */ }, { key: "handleCustomProtocolClick_", value:
    function handleCustomProtocolClick_(e, element, location) {
     ...
     * @return {boolean}
     * @private
     */ }, { key: "handleA2AClick_", value:
    function handleA2AClick_(e, element, location) {
      if (!element.hasAttribute('rel')) {
        return false;
      }
      var relations = element.
      getAttribute('rel').
      split(' ').
      map(function (s) {return s.trim();});
     ...
     * @private
     */ }, { key: "handleNavigation_", value:
    function handleNavigation_(e, element, toLocation, fromLocation) {
     ...
     * @param {string} target
     * @private
     */ }, { key: "removeViewerQueryBeforeNavigation_", value:
    function removeViewerQueryBeforeNavigation_(win, fromLocation, target) {
      dev().info(
      TAG,
      ...
      var noQuery = "".concat(fromLocation.origin).concat(fromLocation.pathname).concat(fromLocation.hash);
     ...
     * @param {!Location} fromLocation
     * @private
     */ }, { key: "handleHashNavigation_", value:
    function handleHashNavigation_(e, toLocation, fromLocation) {var _this2 = this;
      ...
      if (hash) {
        ...
        this.rootNode_. /*OK*/querySelector("a[name=\"".concat(escapedHash, "\"]"));
      }
     ...
     */ }, { key: "registerAnchorMutator", value:
    function registerAnchorMutator(callback, priority) {
     ...
     * @param {number} priority
     */ }, { key: "registerNavigateToMutator", value:
    function registerNavigateToMutator(callback, priority) {
     ...
     * @param {string} hash
     * @private
     */ }, { key: "scrollToElement_", value:
    function scrollToElement_(elem, hash) {var _this3 = this;
      ...
      if (elem) {
        ...
        function () {return _this3.viewport_. /*OK*/scrollIntoView( /** @type {!Element} */(elem));},
        1);

      } else {
        dev().warn(
        TAG, "failed to find element with id=".concat(
        hash, " or a[name=").concat(hash, "]"));
    ...
    function getLocation_() {
      ...
      var baseHref =
      false && !this.isEmbed_ ? this.ampdoc.win.location.href : '';
     ...
     * @return {boolean} Returns true if navigation message was sent to viewer.
     *     Otherwise, returns false.
     */ }, { key: "viewerInterceptsNavigation", value:
    function viewerInterceptsNavigation(url, requestedBy) {
      var viewerHasCapability = this.viewer_.hasCapability(
      'interceptNavigation');
      ...
      return true;
    } }], [{ key: "installAnchorClickInterceptor", value: function installAnchorClickInterceptor(ampdoc, win) {win.document.documentElement.addEventListener('click', maybeExpandUrlParams.bind(null, ampdoc), /* capture */true);} }]);return Navigation;}();
...
function maybeExpandUrlParams(ampdoc, e) {
  var target = closestAncestorElementBySelector( /** @type {!Element} */(
  e.target),
  'A');
  ...
  var newHref = Services.urlReplacementsForDoc(target).expandUrlSync(
  hrefToExpand,
  vars,
  /* opt_allowlist */{
...
function getHrefMinusHash(location) {
  return "".concat(location.origin).concat(location.pathname).concat(location.search);
}
