...
export var WindowPortEmulator = /*#__PURE__*/function () {
   ...
   * @param {!Window} target
   */
  function WindowPortEmulator(win, origin, target) {_classCallCheck(this, WindowPortEmulator);
   ...
   */_createClass(WindowPortEmulator, [{ key: "addEventListener", value:
     ...
     */ }, { key: "start", value:
    function start() {} }]);return WindowPortEmulator;}();
...
export var Messaging = /*#__PURE__*/function () {
   ...
   */
  function Messaging(win, port, opt_isWebview, opt_token, opt_verifyToken) {_classCallCheck(this, Messaging);
    /** @const @private {?Window} */
    this.win_ = win;
   ...
   */_createClass(Messaging, [{ key: "registerHandler", value:
    function registerHandler(messageName, requestHandler) {
     ...
     */ }, { key: "unregisterHandler", value:
    function unregisterHandler(messageName) {
      delete this.messageHandlers_[messageName];
    }

    /**
     * @param {?RequestHandler} requestHandler
     */ }, { key: "setDefaultHandler", value:
    function setDefaultHandler(requestHandler) {
    ...
    function sendRequest(messageName, messageData, awaitResponse) {var _this2 = this;
      var requestId = ++this.requestIdCounter_;
      var promise = undefined;
      if (awaitResponse) {
        promise = new Promise(function (resolve, reject) {
          _this2.waitingForResponse_[requestId] = { resolve: resolve, reject: reject };
        });
      }
      this.sendMessage_(
     ...
     * @param {*} messageData
     * @private
     */ }, { key: "sendResponse_", value:
    function sendResponse_(requestId, messageName, messageData) {
      this.sendMessage_(
      /** @type {!AmpViewerMessage} */({
        app: APP,
        requestid: requestId,
     ...
     * @private
     */ }, { key: "sendResponseError_", value:
    function sendResponseError_(requestId, messageName, reason) {
     ...
     * @private
     */ }, { key: "sendMessage_", value:
    function sendMessage_(message) {
      var /** Object<string, *> */finalMessage = Object.assign(message, {});
     ...
     * @param {!AmpViewerMessage} message
     * @private
     */ }, { key: "handleRequest_", value:
    function handleRequest_(message) {var _this3 = this;
    ...
    function handleResponse_(message) {
      var requestId = message.requestid;
      ...
      if (pending) {
        ...
        if (message.error) {
          ...
          pending.reject(
          new Error("Request ".concat(message.name, " failed: ").concat(message.error)));
     ...
     * @private
     */ }, { key: "logError_", value:
    function logError_(state, opt_data) {
      if (!this.win_) {
        return;
      }
     ...
     * @return {string}
     * @private
     */ }, { key: "errorToString_", value:
    function errorToString_(err) {
      return err ? (err.message ? err.message : String(err)) : 'unknown error';
    } }], [{ key: "initiateHandshakeWithDocument", value: /**
     ...
     */function initiateHandshakeWithDocument(target, opt_token) {return new Promise(function (resolve) {var intervalRef = setInterval(function () {var channel = new MessageChannel();var pollMessage = /** @type {JsonObject} */({ app: APP, name: HANDSHAKE_POLL_MSG });target. /*OK*/postMessage(pollMessage, '*', [channel.port2]);var port = channel.port1;var listener = function listener(event) {var message = parseMessage(event.data);if (!message) {return;}if (message.app === APP && message.name === CHANNEL_OPEN_MSG) {clearInterval(intervalRef);port.removeEventListener('message', listener);var messaging = new Messaging(null, port, /* opt_isWebview */false, opt_token, /* opt_verifyToken */true);messaging.sendResponse_(message.requestid, CHANNEL_OPEN_MSG, null);resolve(messaging);}};port.addEventListener('message', listener);port.start();}, 1000);});} /**
     ...
     * @return {!Promise<!Messaging>}
     */ }, { key: "waitForHandshakeFromDocument", value: function waitForHandshakeFromDocument(source, target, origin, opt_token, opt_cdnProxyRegex) {return new Promise(function (resolve) {var listener = function listener(event) {var message = parseMessage(event.data);if (!message) {return;}if ((event.origin == origin || (opt_cdnProxyRegex && opt_cdnProxyRegex.test(event.origin))) && (!event.source || event.source == target) && message.app === APP && message.name === CHANNEL_OPEN_MSG) {source.removeEventListener('message', listener);var port = new WindowPortEmulator(source, event.origin, target);var messaging = new Messaging(null, port, /* opt_isWebview */false, opt_token, /* opt_verifyToken */true);messaging.sendResponse_(message.requestid, CHANNEL_OPEN_MSG, null);resolve(messaging);}};source.addEventListener('message', listener);});} }]);return Messaging;}();
