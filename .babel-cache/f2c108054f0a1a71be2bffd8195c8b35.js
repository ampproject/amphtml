 ...
 *   gesture.
 */
function Gesture(type, data, time, event) {_classCallCheck(this, Gesture);
  /** @const {string} */
  this.type = type;
  /** @const {DATA} */
  this.data = data;
...
export var Gestures = /*#__PURE__*/function () {
   ...
   */
  function Gestures(element, shouldNotPreventDefault, shouldStopPropagation) {_classCallCheck(this, Gestures);
    /** @private {!Element} */
   ...
   */_createClass(Gestures, [{ key: "cleanup", value:
    function cleanup() {
     ...
     * @template DATA
     */ }, { key: "onGesture", value:
    function onGesture(recognizerConstr, handler) {
     ...
     * @param {function(new:GestureRecognizer, !Gestures)} recognizerConstr
     * @return {boolean}
     */ }, { key: "removeGesture", value:
    function removeGesture(recognizerConstr) {
      var type = new recognizerConstr(this).getType();
      var overserver = this.overservers_[type];
      if (overserver) {
        overserver.removeAll();
        var index = findIndex(this.recognizers_, function (e) {return e.getType() == type;});
        if (index < 0) {
          return false;
        }
     ...
     * @param {!Event} event
     * @private
     */ }, { key: "onTouchCancel_", value:
    function onTouchCancel_(event) {
      for (var i = 0; i < this.recognizers_.length; i++) {
     ...
     * @restricted
     * @visibleForTesting
     */ }, { key: "signalReady_", value:
    function signalReady_(recognizer, offset) {
     ...
     * @restricted
     * @visibleForTesting
     */ }, { key: "signalPending_", value:
    function signalPending_(recognizer, timeLeft) {
      // Somebody got here first.
      if (this.eventing_) {
        recognizer.acceptCancel();
        return;
     ...
     * @restricted
     * @visibleForTesting
     */ }, { key: "signalEnd_", value:
    function signalEnd_(recognizer) {
     ...
     * @restricted
     * @visibleForTesting
     */ }, { key: "signalEmit_", value:
    function signalEmit_(recognizer, data, event) {
      devAssert(
     ...
     * @param {!Event} event
     * @private
     */ }, { key: "afterEvent_", value:
    function afterEvent_(event) {
     ...
     * @param {number} index
     * @private
     */ }, { key: "startEventing_", value:
    function startEventing_(index) {
     ...
     * @param {number} index
     * @private
     */ }, { key: "startTracking_", value:
    function startTracking_(index) {
     ...
     * @param {number} index
     * @private
     */ }, { key: "stopTracking_", value:
    function stopTracking_(index) {
     ...
     * @param {number} index
     * @private
     */ }, { key: "cancelEventing_", value:
    function cancelEventing_(index) {
      this.ready_[index] = 0;
     ...
     * @return {!Gestures}
     */function get(element) {var opt_shouldNotPreventDefault = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : false;var opt_shouldStopPropagation = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : false;var res = element[PROP_];if (!res) {res = new Gestures(element, opt_shouldNotPreventDefault, opt_shouldStopPropagation);element[PROP_] = res;}return res;} }]);return Gestures;}(); /**
 ...
 */export var GestureRecognizer = /*#__PURE__*/function () {/**
   ...
   */function GestureRecognizer(type, manager) {_classCallCheck(this, GestureRecognizer); /** @private @const {string} */
    this.type_ = type;
   ...
   * @return {string}
   */_createClass(GestureRecognizer, [{ key: "getType", value:
    function getType() {
      return this.type_;
    }
     ...
     * in the "ready" state.
     * @param {time} offset
     */ }, { key: "signalReady", value:
    function signalReady(offset) {
     ...
     * "pending" state.
     * @param {time} timeLeft
     */ }, { key: "signalPending", value:
    function signalPending(timeLeft) {
     ...
     * @param {DATA} data
     * @param {?Event} event
     */ }, { key: "signalEmit", value:
    function signalEmit(data, event) {
     ...
     * @return {boolean}
     */ }, { key: "onTouchStart", value:
    function onTouchStart(unusedEvent) {
      return false;
    }
    ...
    function onTouchMove(unusedEvent) {
      return false;
    }
    ...
    function onTouchEnd(unusedEvent) {} }]);return GestureRecognizer;}();
