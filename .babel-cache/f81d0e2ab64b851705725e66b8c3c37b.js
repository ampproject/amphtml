...
export function expandTemplate(template, getter, opt_maxIterations) {
  var maxIterations = opt_maxIterations || 1;var _loop = function _loop(
  i) {
    var matches = 0;
    ...
    if (!matches) {
      return "break";
    }};for (var i = 0; i < maxIterations; i++) {var _ret = _loop(i);if (_ret === "break") break;
  }
  return template;
}
...
export function asyncStringReplace(str, regex, replacer) {
  ...
  return Promise.all(stringBuilder).then(function (resolved) {return resolved.join('');});
}
