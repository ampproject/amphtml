...
export function getFormAsObject(form) {
  ...
  var checkableType = /^(?:checkbox|radio)$/i;var _loop = function _loop(

  i) {
    var input = elements[i];
    var checked = input.checked,multiple = input.multiple,name = input.name,options = input.options,tagName = input.tagName,type = input.type,value = input.value;
    if (
    !name ||
    isDisabled(input) ||
    ...
    data[name].push(value);};for (var i = 0; i < elements.length; i++) {var _ret = _loop(i);if (_ret === "continue") continue;
  }
...
export function isFieldEmpty(field) {
  switch (field.tagName) {
    ...
    case 'SELECT':
      ...
      return false;
    default:
      throw new Error("isFieldEmpty: ".concat(
      field.tagName, " is not a supported field element."));}
