 ...
 * @return {!Location}
 */
var urlAsLocation = function urlAsLocation(url) {return (
...
export function parseUrlDeprecated(url, opt_nocache) {
  ...
  cachedAnchorEl,
  url,
  false || opt_nocache ? null : urlCache);
...
opt_addToFront)
{
  ...
  mainAndQuery[0] + (
  mainAndQuery[1] ?
  opt_addToFront ? "?".concat(
  paramString, "&").concat(mainAndQuery[1]) : "?".concat(
  mainAndQuery[1], "&").concat(paramString) : "?".concat(
  paramString));
  newUrl += mainAndFragment[1] ? "#".concat(mainAndFragment[1]) : '';
  return newUrl;
}
...
export function removeParamsFromSearch(urlSearch, paramName) {
  ...
  var paramRegex = new RegExp("[?&]".concat(paramName, "\\b[^&]*"), 'g');
