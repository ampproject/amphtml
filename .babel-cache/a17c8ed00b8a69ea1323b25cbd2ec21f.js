...
getSourceOrigin,
isProxyOrigin,
resolveRelativeUrl } from "../../../src/url";
 ...
 */
export function timeStrToMillis(time) {var fallbackMs = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : NaN;
...
export function unscaledClientRect(el) {
  var _el$getBoundingClient = el. /*OK*/getBoundingClientRect(),height = _el$getBoundingClient.height,left = _el$getBoundingClient.left,top = _el$getBoundingClient.top,width = _el$getBoundingClient.width;
...
export function getRGBFromCssColorValue(cssValue) {
  ...
  if (!cssValue.match(regexPattern)) {
    user().error(
    'UTILS',
    'getRGBFromCssColorValue expects a parameter in ' + "the form of 'rgba(0, 0, 0, 1)' or 'rgb(0, 0, 0)' but got ".concat(
    cssValue));
 ...
 * @return {?string}
 */
export function getStoryAttributeSrc(element, attribute) {var warn = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : false;
  ...
  if (attrSrc) {
    ...
    user().warn('AMP-STORY', "Expected ".concat(attribute, " attribute on <amp-story>"));
  }
 ...
 * @private @const {string}
 */
var TEXT_BACKGROUND_COLOR_SELECTOR = "[".concat(TEXT_BACKGROUND_COLOR_ATTRIBUTE_NAME, "]");
...
export var maybeMakeProxyUrl = function maybeMakeProxyUrl(url, ampDoc) {
  ...
  var resolvedRelativeUrl = urlService.resolveRelativeUrl(
  url,
