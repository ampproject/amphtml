...
isUserErrorMessage } from "./core/error/message-helpers";
...
export function reportError(error, opt_associatedElement) {
  try {
    ...
    if (!isValidError && false && !false) {
      setTimeout(function () {
        var rethrow = new Error(
    ...
    if (error.messageArray) {
      var elIndex = findIndex(error.messageArray, function (item) {return (item === null || item === void 0) ? (void 0) : item.tagName;});
      if (elIndex > -1) {
 ...
 */
function onError(message, filename, line, col, error) {var _this = this;
...
hasNonAmpJs)
{
  ...
  for (var exp in experiments) {
    ...
    exps.push("".concat(exp, "=").concat(on ? '1' : '0'));
  }
  data['exps'] = exps.join(',');

  if (error) {var _error$associatedElem;
    data['el'] = (((_error$associatedElem = error.associatedElement) === null || _error$associatedElem === void 0) ? (void 0) : _error$associatedElem.tagName) || 'u'; // Unknown
...
function getRootElement_(win) {
  ...
  return (/** @type {!Element} */(root.documentElement || root.body || root));
}
