...
export function toStructuredCloneable(input, init) {
  var newInit = /** @type {!FetchInitDef} */_objectSpread({}, init);
  if (isFormDataWrapper(init.body)) {
    var wrapper = /** @type {!FormDataWrapperInterface} */(init.body);
...
export function getViewerInterceptResponse(win, ampdocSingle, input, init) {
  ...
  var whenUnblocked = init.prerenderSafe ?
  _resolvedPromise2() :
  ...
  var interceptorDisabledForLocalDev =
  init.bypassInterceptorForDev && false;
  ...
  return whenUnblocked.
  then(function () {return viewer.isTrustedViewer();}).
...
export function setupJsonFetchInit(init) {
  ...
  if (fetchInit.method == 'POST' && !isFormDataWrapper(fetchInit.body)) {
    ...
    // as body.
    devAssert(
    allowedJsonBodyTypes_.some(function (test) {return test(fetchInit.body);}));
...
export function assertSuccess(response) {
  return new Promise(function (resolve) {
    ...
    var err = user().createError("HTTP error ".concat(status));
