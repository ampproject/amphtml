var _template = ["<i-amphtml-sizer class=i-amphtml-sizer slot=i-amphtml-svc><img alt=\"\" role=presentation aria-hidden=true class=i-amphtml-intrinsic-sizer></i-amphtml-sizer>"]; /**
...
isLayoutSizeDefined,
parseLayout,
parseLength } from "./core/dom/layout";
 ...
 * @return {!Layout}
 */
export function applyStaticLayout(element) {var fixIeIntrinsic = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : false;
  ...
  if (completedLayoutAttr) {
    var _layout = /** @type {!Layout} */(
    devAssert(parseLayout(completedLayoutAttr)));

    if (
    (_layout == Layout.RESPONSIVE || _layout == Layout.INTRINSIC) &&
  ...
  if (layout == Layout.NODISPLAY) {
    ...
    if (shouldUseAspectRatioCss(toWin(element.ownerDocument.defaultView))) {
      ...
      element,
      'aspect-ratio', "".concat(
      getLengthNumeral(width), "/").concat(getLengthNumeral(height)));
    ...
    var _sizer = htmlFor(element)(_template);
    ...
    'src',
    !false && fixIeIntrinsic && element.ownerDocument ?
    transparentPng(
    element.ownerDocument,
    devAssertNumber(getLengthNumeral(width)),
    devAssertNumber(getLengthNumeral(height))) : "data:image/svg+xml;charset=utf-8,<svg height=\"".concat(

    height, "\" width=\"").concat(width, "\" xmlns=\"http://www.w3.org/2000/svg\" version=\"1.1\"/>"));
...
function getEffectiveLayoutInternal(element) {
  ...
  return { layout: layout, width: width, height: height };
}
